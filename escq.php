...
$sql="SELECT * FROM `sdms_ticket` WHERE `isaccepted`=0 AND DATE(created)='".$checkdate."' AND isrejected<>'1' AND isquery='1' AND ticket_id>=4100 AND status='open'";
...
<p>Query Number ".$row['ticket_id']." assigned to ".$deppochrow['firstname'].' '.$deppochrow['lastname'].", is awaiting action/update to complainant from your end since query lodgment date ".date('d,M Y',strtotime($row['created'])).".  This incident is being escalated according to the Commission approved Service Desk Guideline 2019. </p>
...
$sql="SELECT * FROM `sdms_ticket` WHERE `isaccepted`=1 AND DATE(created)='".$checkdate."' AND isrejected<>'1' AND ticket_id>=4100 AND status='open' AND isquery='1' AND ticket_id NOT IN (select ticket_id from sdms_ticket_thread where thread_type='R')";
$result = mysqli_query($conn, $sql);
