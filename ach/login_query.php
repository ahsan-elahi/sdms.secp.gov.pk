...
if($_POST) {
	 ...
	 /*********************************************************************/
		...
		if(($user=Client::login(trim($_POST['lticket']), trim($_POST['lemail']), null, $errors))) {
			...
			@header('Location: queries.php?task=web&id='.$user->getTicketID());
			require_once('queries.php'); //Just in case of 'header already sent' error.
			exit;
		} elseif(!$errors['err']) {
...
require(CLIENTINC_DIR.'login_query.inc.php');
