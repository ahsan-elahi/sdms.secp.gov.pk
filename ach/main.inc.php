...
ini_set('display_errors', 'On');
    ...
    elseif(file_exists(INCLUDE_DIR.'settings.php')) { //OLD config file.. v 1.6 RC5
        ...
        if(!strcasecmp(basename($_SERVER['SCRIPT_NAME']), 'settings.php'))
            die('Please rename config file include/settings.php to include/sdms-config.php to continue!');
    } elseif(file_exists(INCLUDE_DIR.'sdms-config.php')) //NEW config file v 1.6 stable ++
        $configfile=INCLUDE_DIR.'sdms-config.php';
   ...
   //Path separator
	...
	define('SERVICE_TABLE',TABLE_PREFIX.'service');
	...
	define('TICKET_EVIDENCE_TABLE',TABLE_PREFIX.'ticket_evidence');
	
	define('TICKET_SECURITIES_MARKETS_TABLE',TABLE_PREFIX.'securities_markets');
	define('TICKET_INSURANCE_TABLE',TABLE_PREFIX.'insurance');
	define('TICKET_SPECIALIZED_COMPANIES_TABLE',TABLE_PREFIX.'specialized_companies');
	define('TICKET_E_SERVICES_TABLE',TABLE_PREFIX.'e_services');
	define('TICKET_C_AND_C_TABLE',TABLE_PREFIX.'c_and_c');
	define('TICKET_AGAINTS_CRO_TABLE',TABLE_PREFIX.'againts_cro');
	...
	define('DISTRICTS_TABLE',TABLE_PREFIX.'districts'); 
	define('TEHSILS_TABLE',TABLE_PREFIX.'tehsils');
	define('SERVICES_TABLE',TABLE_PREFIX.'service');
	...
	define('STAFF_LOCATION',TABLE_PREFIX.'staff_location');
	
	define('AGENCY_TEHSIL',TABLE_PREFIX.'agency_tehsils');
    ...
    if($ferror) { //Fatal error
        ...
        $msg=$ferror."\n\n".THISPAGE;
       // Mailer::sendmail(ADMIN_EMAIL, 'osTicket Fatal Error', $msg, sprintf('"osComplaint Alerts"<%s>', ADMIN_EMAIL));
