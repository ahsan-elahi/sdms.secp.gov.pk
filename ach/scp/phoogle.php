...
function addzips($address){
$reader = new XMLReader();

		 $apiURL = "http://maps.googleapis.com/maps/api/geocode/xml?address=".$address."&sensor=false";
		$reader->open($apiURL);
							while($reader->read())
							 {
	...
	function addAddress($address,$htmlMessage=null){
	 if (!is_string($address)){
	  ...
	  $reader = new XMLReader();
		 ...
		 $apiURL = "http://maps.googleapis.com/maps/api/geocode/xml?address=".urlencode($address)."&sensor=false";
		$reader->open($apiURL);
							while($reader->read())
							 {
								if ($reader->nodeType == XMLReader::ELEMENT) 
							...
							if($lat!="" & $lng!="")
							{
										...
										$this->validPoints[$pointer]['long']=$lng;
										$this->validPoints[$pointer]['passedAddress'] = $address;
										$this->validPoints[$pointer]['htmlMessage'] = $htmlMessage;
							}			
							
		/*$apiURL = "http://where.yahooapis.com/geocode?appid=dj0yJmk9YmF1VGVPWDBGZzdIJmQ9WVdrOWVuWkNWWEpqTXpBbWNHbzlNVGcyTURjM05EZzJNZy0tJnM9Y29uc3VtZXJzZWNyZXQmeD1hNg--&q=";
		$addressData = file_get_contents($apiURL.urlencode($address));
		$results = $this->xml2array($addressData);
		
		if (empty($results['ResultSet']['Result']['quality'])){
