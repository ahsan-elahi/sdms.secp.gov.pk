...
function category_tree($catid){
	...
	$sql_topics = "SELECT * FROM `sdms_help_topic` WHERE dept_id = '1' AND topic_pid = '".$catid."' AND isnature = '0' ";
	...
	while($row = mysql_fetch_array($res_topics)){
		...
		{ 
			echo '<ul>';
				...
				echo '<li>' .$row['topic_id'].') '.$row['topic'];
...
function count_complaint($topic_id){
	...
	echo '<br>Ticket in '.$topic_id.' ='. $total_complaints;
		
	$sql_inner_topics = "SELECT * FROM `sdms_help_topic` WHERE dept_id = '1' AND topic_pid = '".$topic_id."' AND isnature = '0' ";
	...
	//echo '<br>'.$num_inner_topics.'<br>';
		...
		count_complaint($row_inner_topics['topic_id']);
		}
