...
$status=null;
if($_REQUEST['id'] && !($status=Status::lookup($_REQUEST['id'])))
...
if($_POST){
    switch(strtolower($_POST['do'])){
        case 'update':
            ...
            break;
        case 'create':
            if(($id=Status::create($_POST,$errors))){
                $msg='Complaint Status added successfully';
                $_REQUEST['a']=null;
            }elseif(!$errors['err']){
                $errors['err']='Unable to add Complaint Status. Correct error(s) below and try again.';
            }
            break;
        case 'mass_process':
