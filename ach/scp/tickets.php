...
//$thisstaff= new Staff($_SESSION['_staff']['userID']);
if(!$_SESSION['counter'])
{
	...
	elseif( $thisstaff->getGroupId()==9)
	{
	$_SESSION['counter'] = 1;	
	@header("Location: departmental_summary_report.php");
	}
	elseif( $thisstaff->onChairman()==1)
	{
	$_SESSION['counter'] = 1;	
	@header("Location: tickets.php");
	}
	elseif($thisstaff->getGroupId()==8){
		$_SESSION['counter'] = 1;	
	@header("Location: tickets.php?status=assigned");
	}
...
if($_REQUEST['subdeptviewid']) {
	 ...
	 if(!($ticket=Ticket::lookup($_REQUEST['subdeptviewid'])))
         $errors['err']='Unknown or invalid ticket ID';
...
//            if(!$errors && ($response_to_department=$ticket->postDepartment($vars, $errors)) && !$ticket->pdfExport_Department($vars['response'])) {
	...
	/*===================================================================================================================================================*/
		case 'reply':
            ...
            $wasOpen =($ticket->isOpen());
            //If no error...do the do.
			$_POST['complaint_status'] = $ticket->complaint_status_title();
            $vars = $_POST;
            ...
            if(!$thisstaff->canTransferTickets())
                ...
                if(!$errors && $ticket->transfer($_POST['deptId'], $_POST['transfer_comments'])) {
                    ...
                    if(!$ticket->checkStaffAccess($thisstaff))
						...
						if($_POST['ishandel_reassign']=='1')
					{
					 $ishandel_reassign  = "UPDATE `sdms_ticket` SET `ishandel_reassign`='1' Where `ticket_id`='".$_POST['ticket_id']."'";
					 mysql_query($ishandel_reassign);
					}
            ...
            switch(strtolower($_POST['do'])):
                ...
                case 'reopen':
                    ...
                    if(!$thisstaff->canCloseTickets() && !$thisstaff->canCreateTickets()) {
                        ...
                        $ticket->logNote('Complaint Reopened', $note,'6', $thisstaff);
                ...
                case 'claim':
				 ...
				 case 'claim':
                   if($ticket->copy_dept_data($_REQUEST['id'])) {
                        $msg = 'Complaint is now Updateded successfull!';
                    } else {
                        $errors['err'] = 'Problems Copying the Complaint Details. Try Again';
                    }
                    break;
...
/*if($thisstaff->canCreateTickets() && !$thisstaff->isAdmin()) {
    $nav->addSubMenu(array('desc'=>'New Complaints',
	 ...
	 $nav->addSubMenu(array('desc'=>'All Complaints',
...
if($cfg->showAssignedTickets() && !$thisstaff->isFocalPerson()) {
    $nav->addSubMenu(array('desc'=>'Accepted ('.number_format($stats['assigned']).')',
                            'title'=>'Accepted',
                            'href'=>'tickets.php?status=assigned',
                            'iconclass'=>'Ticket',
							'class'=>'ibw-Open_Complaints'),
                        (!$_REQUEST['status'] || $_REQUEST['status']=='assigned'));} 
else {
    if($stats ) {
...
if($thisstaff->getGroupId()==8 && $_REQUEST['status']==''){
...
if($ticket) {
	...
	elseif($_REQUEST['status'] == 'nearoverdue')
	$inc = 'tickets-nearoverdue.inc.php';
	elseif($_REQUEST['action'] == 'subdept')
	$inc = 'tickets-subdept.inc.php';	
