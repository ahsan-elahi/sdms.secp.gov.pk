...
$(document).ready(function(){
   ...
   /* $('#actions input:submit.btn').bind('click', function(e) {

        var formObj = $(this).closest('form');
        e.preventDefault();
        if($('.dialog_show#ba_btn_'+this.name+' p#'+this.name+'-confirm').length == 0) {
            alert('Unknown action '+this.name+' - get technical help.');
        } else if(checkbox_checker(formObj, 1)) {
			
            var action = this.name;
            $('.dialog_show#ba_btn_'+this.name).undelegate('.confirm');
            $('.dialog_show#ba_btn_'+this.name).delegate('input.confirm', 'click.confirm', function(e) {
                e.preventDefault();
                $('.dialog_show#ba_btn_'+this.name).hide();
                $('#overlay').hide();
            ...
            $('#overlay').show();
            $('.dialog_show#ba_btn_'+this.name+'.confirm-action').hide();
            $('.dialog_show#ba_btn_'+this.name+' p#'+this.name+'-confirm')
            .show()
    ...
    $('.dialog_show').delegate('input.close, a.close', 'click', function(e) {
        e.preventDefault();
        $(this).parents('div.dialog_show').hide()
        $('#overlay').hide();
    ...
    /* advanced search */
    $('.dialog_show#advanced-search').css({
        top  : ($(window).height() / 6),
        left : ($(window).width() / 2 - 300)
    });
