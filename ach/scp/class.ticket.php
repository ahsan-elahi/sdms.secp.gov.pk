...
//print_r ( $this->getThread()->getEntries($type, $order));exit;
	...
	CURLOPT_URL => 'http://119.160.92.2:7700/sendsms_url.html?Username=03028504208&Password=123.123&From=8181&To='.$selectednums.'&Message='.$message.'',
...
*/     
        ...
        if(!db_query($sql) || !db_affected_rows())
    {
			...
			elseif($vars['isquery']==0){
				$deptId=$vars['deptId'];
			/*$sql_evidence="Update ".TICKET_EVIDENCE_TABLE." SET created = NOW() , evidence_1='".$vars['evidence_1']."',evidence_2='".$vars['evidence_2']."',evidence_3='".$vars['evidence_3']."', evidence_4='".$vars['evidence_4']."' , evidence_5='".$vars['evidence_5']."' , evidence_6='".$vars['evidence_6']."',ticket_id='".$this->getId()."'";
			db_query($sql_evidence);*/
				if($deptId==2){
				$sql_dept_details = "Select * from sdms_ticket_capital_markets where  WHERE `complaint_id` = '".$this->getId()."'";
				...
				$sql_dept_details = "Select * from sdms_ticket_insurance where  WHERE `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				...
				$sql_dept_details = "Select * from sdms_ticket_scd where  WHERE `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				...
				$sql_dept_details = "Select * from sdms_ticket_e_services where  WHERE `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				...
				$sql_dept_details = "Select * from sdms_ticket_cr where  WHERE `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				
				$sql_c_and_c="Update `sdms_ticket_cr` SET `cr_status`='".$_POST['cr_status']."', `cr_company_title`='".addslashes($_POST['cr_company_title'])."', `cr_folio_no`='".$_POST['cr_folio_no']."', `cr_no_of_shares`='".$_POST['cr_no_of_shares']."',`cr_cro`='".$_POST['cr_cro']."',`cr_process_name`='".$_POST['cr_process_name']."' WHERE `complaint_id`= '".$this->getId()."'";
				db_query($sql_c_and_c);	
				}else{
				$sql_c_and_c="INSERT INTO `sdms_ticket_cr` (`cr_status`, `cr_company_title`, `cr_folio_no`, `cr_no_of_shares`,`cr_cro`,`cr_process_name`, `complaint_id`) VALUES ( '".$_POST['cr_status']."', '".addslashes($_POST['cr_company_title'])."', '".$_POST['cr_folio_no']."', '".$_POST['cr_no_of_shares']."','".$_POST['cr_cro']."','".$_POST['cr_process_name']."', '".$this->getId()."')";
				db_query($sql_c_and_c);						
				}
				
				}
				else if($deptId==18){
				$sql_dept_details = "Select * from sdms_ticket_cs where  WHERE `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
...
//echo $ticket;exit;
     ...
     */
		...
		*/	 
		...
		return null;
			...
			$sql_evidence="INSERT INTO ".TICKET_EVIDENCE_TABLE." SET created = NOW() , evidence_1='".$vars['evidence_1']."',evidence_2='".$vars['evidence_2']."',evidence_3='".$vars['evidence_3']."', evidence_4='".$vars['evidence_4']."' , evidence_5='".$vars['evidence_5']."' , evidence_6='".$vars['evidence_6']."',ticket_id='".$id."'";
			db_query($sql_evidence);
			...
			$sql_c_and_c="INSERT INTO `sdms_ticket_cr` (`cr_status`, `cr_company_title`, `cr_folio_no`, `cr_no_of_shares`,`cr_cro`,`cr_process_name`, `complaint_id`) VALUES ( '".$_POST['cr_status']."', '".addslashes($_POST['cr_company_title'])."', '".$_POST['cr_folio_no']."', '".$_POST['cr_no_of_shares']."','".$_POST['cr_cro']."','".$_POST['cr_process_name']."', '".$id."')";
			db_query($sql_c_and_c);						
			}
			else if($deptId==18){
		...
		$message .= 'Your complaint has reached us and is currently under review. You can track your complaint by the following complaint number '.$extId.' and pincode is '.$pincode.'. You should expect an update in this regard shortly.<br>';
		}
		$message .= "Thank you for your patience.<br>Regards,<br>SECP Service Desk<br>UAN: 080088008<br>Please do not respond to this sms";
		...
		curl_setopt_array($curl, array(
		CURLOPT_RETURNTRANSFER => 1,
		CURLOPT_URL => 'http://119.160.92.2:7700/sendsms_url.html?Username=03028504208&Password=123.123&From=8181&To='.$vars['phone'].'&Message='.$message.'',
    ...
    function open($vars, &$errors) {
        ...
        if($vars['source'] && !in_array($vars['source'],array('At Counter','From Chief Justice','By Email','Through Call','By Post','By Fax','Online Complaint')))
...
if($row_complaint_info['isquery']=='0'){
	...
	$subject = "Query received for handling by relevant department of SECP";
		...
		// Sms Code 
		$message = "Dear Sir/Miss,<br>Your ".$ticket_type." titled ".$row_complaint_info['subject']." has been accepted for handling by the ".$row_dept_info['dept_name']." department. You can track your complaint by using the following complaint number ".$row_complaint_info['ticketID']." and pincode is ".$row_complaint_info['ticketpin'].".<br>Regards,<br>SECP Service Desk<br>Toll Free: 080088008<br>Email: queries@secp.gov.pk<br>complaints@secp.gov.pk<br>Please do not respond to this sms";
		$email_message = $message; 		
		$message = urlencode($message);
		$curl = curl_init();
		curl_setopt_array($curl, array(
		CURLOPT_RETURNTRANSFER => 1,
		CURLOPT_URL => 'http://119.160.92.2:7700/sendsms_url.html?Username=03028504208&Password=123.123&From=8181&To='.$row_complaint_info['phone'].'&Message='.$message.'',
		CURLOPT_USERAGENT => 'SDMS SECP'
		));
		$resp = curl_exec($curl);
		curl_close($curl);
