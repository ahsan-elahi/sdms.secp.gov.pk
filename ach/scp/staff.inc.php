...
if(!$thisstaff || !is_object($thisstaff) || !$thisstaff->getId() || !$thisstaff->isValid()){
    $msg=(!$thisstaff || !$thisstaff->isValid())?'':'Session timed out due to inactivity';
    staffLoginPage($msg);
    exit;
}
...
if ($_POST  && !$ost->checkCSRFToken()) {
	...
	?>
    <script language="javascript">window.location='http://feedback.khyber.gov.pk/ach/scp/login.php';</script>
    <?php 
    exit;
}
...
$ost->setPageTitle('SECP GRS CMS :: Staff Control Panel');
