...
class Ticket {
    ...
    function load($id=0) {
        if(!$id && !($id=$this->getId()))
            return false;
        $sql='SELECT  ticket.*, lock_id, priority_desc '
      ...
      $sql1='SELECT  ticket.*, stat.*, dist.*,tehs.*,agency_tehsil.*,service.*' 	 	
            .' FROM '.TICKET_TABLE.' ticket '
            .' LEFT JOIN '.STATUS_TABLE.' stat ON (ticket.complaint_status=stat.status_id) '
            .' LEFT JOIN '.DISTRICTS_TABLE.' dist ON (ticket.district=dist.District_ID) '
            .' LEFT JOIN '.TEHSILS_TABLE.' tehs ON (ticket.tehsil=tehs.Tehsil_ID) '	
			.' LEFT JOIN '.AGENCY_TEHSIL.' agency_tehsil ON (ticket.AgencyTehsil_ID=agency_tehsil.AgencyTehsil_ID) '				
            .' LEFT JOIN '.SERVICES_TABLE.' service ON (ticket.service_id=service.service_id) '	
            .' WHERE ticket.ticket_id='.db_input($id)
            .' GROUP BY ticket.ticket_id';
...
if(!($res1=db_query($sql1)) || !db_num_rows($res1))
		...
		$this->ht1 = db_fetch_array($res1);
        $this->id       = $this->ht['ticket_id'];
        $this->number   = $this->ht['ticketID'];
		$this->ticketpin= $this->ht['ticketpin'];
		$this->ticketnic= $this->ht['nic'];
		$this->ticketgender= $this->ht['gender'];
		$this->ticketuseradd= $this->ht['user_address'];
	...
	function getName_Title() {
        return $this->ht['name_title'];
    }
    function getName() {
        return $this->ht['name'];
    }
	function getAcceptStatus() {
        return $this->ht['isaccepted'];
    }
	function getRelation_Title() {
        return $this->ht['relation_title'];
    }
    function getRelation_Name() {
        return $this->ht['relation_name'];
    }
	 function getNic() {
        return $this->ht['nic'];
    }
	 function getGender() {
        return $this->ht['gender'];
    }
	
	function checkcategory($ticket_id,$dept_id,$topic_id){
		if($topic_id==43)
		{
		return false;
		}else{
		$sql_get_topics='Select * from sdms_ticket where ticket_id = '.$ticket_id.' AND topic_id IN (SELECT topic_id FROM '.TOPIC_TABLE.' WHERE dept_id="'.$dept_id.'" and isnature = 0) ';
		//if($ticket_id == '73810')
		//{
		//echo $sql_get_topics;}
		$res_get_topic=mysql_query($sql_get_topics);
		if(mysql_num_rows($res_get_topic)>0)
		{
		return true;
		}
		else{
	...
	function getApplicant_Location() {
        return $this->ht['applicant_location'];
    }
	function getDistrictID() {
        return $this->ht1['District_ID'];
    }
	function getServiceID() {
        return $this->ht1['service_id'];
    }
	function getServiceTitle() {
        return $this->ht1['service_title'];
    }
	function getDistrict() {
        return $this->ht1['District'];
    }
	function getOccupation() {
        return $this->ht['occupation'];
    }
	function getage() {
        return $this->ht['age'];
    }
	function getAgainstMode() {
        return $this->ht['comp_against_mode'];
    }
	function getAgainstDeptName() {
        $sql_agasint_dept="Select * from sdms_department where dept_id ='". $this->ht['comp_against_dept']."'";
   $res_agasint_dept=db_query($sql_agasint_dept);
       $row_agasint_dept = db_fetch_array($res_agasint_dept);
		
        return $row_agasint_dept['dept_name'];
    }
	function getTehsil() {
        return $this->ht1['Tehsil_Name'];
    }	
	function getTehsilID() {
        return $this->ht1['Tehsil_ID'];
    }
	function getAgencyTehsilTitle() {
        return $this->ht1['AgencyTehsil_Name'];
    }
	function getAgencyTehsilID() {
        return $this->ht1['AgencyTehsil_ID'];
    }	
	function getApplicant_Address() {
        return $this->ht['applicant_address'];
	}
	function getReceipt() {
        return $this->ht['receipt'];
	}	
	function getDiary_No() {
        return $this->ht['diary_nos'];
	}
	function getPrevious_Complaint() {
        return $this->ht['pre_complaint'];
	}
	
	function getComplaintDetials() {
		$sql_complaint_details="Select * from sdms_ticket_thread where ticket_id='".$this->getId()."' ORDER By id limit 0,1";
		$res_complaint_details=mysql_query($sql_complaint_details);
		$row_complaint_details=mysql_fetch_array($res_complaint_details);
		
		return $row_complaint_details['body'];
	}
	
	
	//-----------------------------------------------//
	function getcomp_against() {
        return $this->ht['comp_against'];
    }
	function getcomp_against_fname() {
        return $this->ht['against_fname'];
    }
	
	function against_phone() {
        return $this->ht['against_phone'];
    }
	function against_cnic() {
        return $this->ht['against_cnic'];
    }
	function against_email() {
        return $this->ht['against_email'];
    }
	function getaddress() {
        return $this->ht['address'];
    }
	function getcomp_against_dept() {
        return $this->ht['comp_against_dept'];
    }
	function getlocation() {
        return $this->ht['location'];
    }
    function getSubject() {
	...
	return $this->ht1['status_title'];
	}
	
	function complaint_status_title(){
	return $this->ht['complaint_status'];
	}
	
    function getPincode() {
        return $this->ht['ticketpin'];
    }
    function getDeptId() {
    ...
    function getUpdateInfo() {
        $info=array('name'  =>  $this->getName(),
					...
					'name_title'=> $this->getName_Title(),	
					
					'relation_title'=> $this->getRelation_Title(),	
					
					'relation_name'=> $this->getRelation_Name(),
					...
					'applicant_location'=>$this->getApplicant_Location(),
					
					'district'  =>  $this->getDistrictID(),
					
					'service_title'  =>  $this->getServiceTitle(),
					
					'service'  =>  $this->getServiceID(),
					
					'tehsil'  =>  $this->getTehsilID(),
					
					'agencytehsil'  =>  $this->getAgencyTehsilID(),
					
					'applicant_address'=>$this->getApplicant_Address(),
					
					'diary_nos'=>$this->getDiary_No(),
					
					'pre_complaint'=>$this->getPrevious_Complaint(),
                    ...
                    'comp_against'  =>  $this->getcomp_against(),
					
					'against_fname'  =>  $this->getcomp_against_fname(),
					
					'against_phone'  =>  $this->against_phone(),
					
					'against_cnic'  =>  $this->against_cnic(),
					
					'against_email'  =>  $this->against_email(),				
									
					'location'  =>  $this->getlocation(),
					
					'address'=>$this->getaddress(),
					
					'complaint_status_title'=>$this->complaint_status_title(),
    ...
    function getClient() {
        if(!$this->client)
            $this->client = Client::lookup($this->getExtId(), $this->getNic());
        return $this->client;
    }
    function getStaffId() {
...
//print_r ( $this->getThread()->getEntries($type, $order));exit;
    ...
    function setDeptId($deptId) {
        ...
        //ahsan code to un match the smae department due to admin dept transfer
    ...
    function close() {
        global $thisstaff;
        $sql='UPDATE '.TICKET_TABLE.' SET closed=NOW(),isoverdue=0, duedate=NULL,isrejected = 0, updated=NOW(), status='.db_input('closed');
		...
		$sql_info='Select * From '.TICKET_TABLE.' WHERE ticket_id='.db_input($this->getId());
		$res_info=db_query($sql_info);
        $row_info = db_fetch_array($res_info);
		
		
		if($row_info['isquery']=='1')
		{
		$type_of_ticket = 'query';
		}else
		{
		$type_of_ticket = 'complaint';
		}
		
		$sql_fetchstatus = "Select * from sdms_status where status_id='".$row_info['complaint_status']."'";
		  $res_fetchstatus = mysql_query($sql_fetchstatus);
		  $row_fetchstatus = mysql_fetch_array($res_fetchstatus);
		  
		if($row_fetchstatus['p_id']=='2')
		{
		$status_of_ticket = 'Closed';
		}else
		{
		$status_of_ticket = 'Resolved';
		}
		
		// Sms to Client
		$selectednums = $row_info['phone'];
		$message='Your '.$type_of_ticket.' no '.$row_info['ticketID'].' has been '.$status_of_ticket.'.  If you have any questions, please call us on 080088008.  Please do not respond to this sms.';
		$message = urlencode($message);
		$curl = curl_init();
		curl_setopt_array($curl, array(
	CURLOPT_RETURNTRANSFER => 1,
	/*CURLOPT_URL => 'https://connect.jazzcmt.com/sendsms_url.html?Username=03028504208&Password=123.123&From=8181&To='.$selectednums.'&Message='.$message.'',*/
    CURLOPT_URL => 'http://smsctp1.eocean.us:24555/api?action=sendmessage&username=SECP&password=P@87654&recipient='.$selectednums.'&originator=8181&messagedata='.$message.'',
	CURLOPT_USERAGENT => 'SDMS SECP'
		));
		...
		if($row_info['complaint_status']=='23' && $row_info['isquery']=='0')
		{
		//Email to Client
		$to = $row_info['email'];
		$email='alert@secp.gov.pk';
		$subject= "Complaint has been Resolved";
		$message="Dear ".$row_info['name']."
Your complaint number ".$row_info['ticketID']." has been resolved.  To provide us feedback, please click on the link provided below
<a href='http://sdms.secp.gov.pk/feedback.php?id=".$row_info['ticket_id']."'>Click Here For Feedback</a>
SDMS SECP";
    ...
    function reopen($isanswered=0) {
        ...
        $sql='UPDATE '.TICKET_TABLE.' SET updated=NOW(), reopened=NOW() '
			...
			.' ,complaint_status='.db_input($status)			
            .' WHERE ticket_id='.db_input($this->getId());
    ...
    function onNewTicket($message, $autorespond=true, $alertstaff=true) {
        ...
        if($autorespond && $email && $cfg->autoRespONNewTicket()
			...
			$msg = str_replace("#%{ticket.ticketpin}",$this->getPincode(),$msg);
    ...
    function onOpenLimit($sendNotice=true) {
        ...
        $msg=sprintf('Max open Complaints (%d) reached  for %s ', $cfg->getMaxOpenTickets(), $this->getEmail());
        $ost->logWarning('Max. Open Complaint Limit ('.$this->getEmail().')', $msg);
        ...
        $alert='Max. open Complaints reached for '.$this->getEmail()."\n"
              .'Open Complaint: '.$client->getNumOpenTickets()."\n"
    ...
    function onMessage($autorespond=true, $alert=true) {
       ...
       /**********   double check auto-response  ************/
        ...
        if($email && $tpl && ($msg=$tpl->getNewMessageAutorepMsgTemplate())) {
            $msg = $this->replaceVars($msg, array('signature' => ($dept && $dept->isPublic())?$dept->getSignature():''));
            //Reply separator tag.
            if($cfg->stripQuotedReply() && ($tag=$cfg->getReplySeparator()))
                $msg['body'] ="\n$tag\n\n".$msg['body'];
            $email->sendAutoReply($this->getEmail(), $msg['subj'], $msg['body']);
    ...
    function onAssign($assignee, $comments,$status, $alert=true) {
        ...
        $this->reload();
        $comments = $comments?$comments:'Complaint assignment';
        $assigner = $thisstaff?$thisstaff:'SYSTEM (Auto Assignment)';
		
        //Log an internal note - no alerts on the internal note.
		
		$this->logNote('Complaint Assigned to '.$assignee->getName(), $comments,$status, $assigner, false);
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function postMessage($vars, $origin='', $alerts=true) {
        ...
        if(!strcasecmp($origin, 'Email') && $cfg->stripQuotedReply() && ($tag=$cfg->getReplySeparator()) && strpos($vars['message'], $tag))
    ...
    /*Theread Section 1)postNote 2)postAffidavte 3)assign 4)transfer 5)postReply */
	 //Legal Summy or Inter Note.
  	 function postNote($vars, &$errors, $poster,$status, $alert=true) {
     ...
     //Attached Affidavte or postaffidavite	
	 function postAffidavte($vars, &$errors, $alert = true) {
        ...
        if(!($affidavite = $this->getThread()->addAffidavte($vars, $errors)))
            return null;
        ...
        if($tpl && ($msg=$tpl->getReplyMsgTemplate()) && $email) {
            ...
            elseif($vars['signature']=='dept' && $dept && $dept->isPublic())
                ...
                $signature='';
            $msg = $this->replaceVars($msg,
                    array('response' => $affidavite, 'signature' => $signature, 'staff' => $thisstaff));
            if($cfg->stripQuotedReply() && ($tag=$cfg->getReplySeparator()))
                $msg['body'] ="\n$tag\n\n".$msg['body'];
            //Set attachments if emailing.
            $attachments = $cfg->emailAttachments()?$affidavite->getAttachments():array();
   ...
   function assign($assignId, $note,$status, $alert=true) {
        ...
        if($assignId[0]=='t') {
			...
			$rv = $this->assignToStaff($id, $note,$status, $alert);
	   }
        return $rv;
	  }	
	  
	 //Dept Tranfer...with alert.. done by staff
     function postDepartment($vars, &$errors, $alert = true) {
        global $thisstaff, $cfg;
        if(!$vars['poster'] && $thisstaff)
            $vars['poster'] = $thisstaff->getName();
        if(!$vars['staffId'] && $thisstaff)
            $vars['staffId'] = $thisstaff->getId();
        if(!($response_to_department = $this->getThread()->addResponse_to_Department($vars, $errors)))
            return null;
        //Set status - if checked.
        if(isset($vars['reply_ticket_status']) && $vars['reply_ticket_status'])
            $this->setStatus($vars['reply_ticket_status']);
        ...
        if(!$alert) return $response_to_department;
        $dept = $this->getDept();
        if(!($tpl = $dept->getTemplate()))
            $tpl= $cfg->getDefaultTemplate();
        if(!$dept || !($email=$dept->getEmail()))
        ...
        if($tpl && ($msg=$tpl->getReplyMsgTemplate()) && $email) {
            ...
            elseif($vars['signature']=='dept' && $dept && $dept->isPublic())
                ...
                $signature='';
            $msg = $this->replaceVars($msg,
                    array('response' => $response_to_department, 'signature' => $signature, 'staff' => $thisstaff));
            if($cfg->stripQuotedReply() && ($tag=$cfg->getReplySeparator()))
                $msg['body'] ="\n$tag\n\n".$msg['body'];
            //Set attachments if emailing.
            $attachments = $cfg->emailAttachments()?$response_to_department->getAttachments():array();
            //TODO: setup  5 param (options... e.g mid trackable on replies)
            $email->send($this->getEmail(), $msg['subj'], $msg['body'], $attachments);
        }
        return $response_to_department;
    }	
	 //Complaint Replay to End User  or reply.
	 ...
	 //Complaint Accepted to End User.
	 function postAccepted($vars, &$errors, $alert = true) {
        global $thisstaff, $cfg;
        if(!$vars['poster'] && $thisstaff)
            $vars['poster'] = $thisstaff->getName();
        if(!$vars['staffId'] && $thisstaff)
	 ...
	 function transfer($deptId, $comments, $alert = true) {
        ...
        $title='Complaint transfered from '.$currentDept.' to '.$this->getDeptName();
	 ...
	 //================================================================//
     function assignToStaff($staff, $note,$status, $alert=true) {
        ...
        $this->onAssign($staff, $note,$status, $alert);
    ...
    function logActivity($title, $note) {
        ...
        if(!$cfg || !$cfg->logTicketActivity())
            return 0;
			$stauts= $this->complaint_status_title();
        return $this->logNote($title, $note,$stauts ,'SYSTEM', false);
    }
    ...
    //Insert Internal Notes/Execute on Assign complaint to Members 
    function logNote($title, $note, $status, $poster='SYSTEM', $alert=true) {
        $errors = array();
					
			
        return $this->postNote(array('title' => $title,'complaint_status'=>$status, 'note' => $note),$errors,$poster,$status,$alert);
    }
    ...
    function pdfExport($psize='Letter', $notes=false) {
        ...
        $name='Complaint-'.$this->getExtId().'.pdf';
        $pdf->Output($name, 'I');
        //Remember what the user selected - for autoselect on the next print.
        $_SESSION['PAPER_SIZE'] = $psize;
        exit;
    }
	function updatethread($subject,$des,$t_id) {
		$sql='Update '.TICKET_THREAD_TABLE.' SET title="'.$subject.'" , body="'.$des.'" Where id='.$t_id;
 		db_query($sql);
		
		$sql2='Update '.TICKET_TABLE.' SET subject="'.$subject.'"  WHERE ticket_id='.db_input($this->getId());
 		db_query($sql2);
		
		@header('Location: tickets.php?id='.$this->getId());
		exit;
		return true;
    }

	//Print Department Compunication... export the Department Compunication as PDF.
    function pdfExport_Department($responce) {
		$psize='Letter';
		 $notes=false;
        $pdf = new Ticket2PDF_Depart($this, $psize, $notes,$responce);
        $name='Complaint-'.$this->getExtId().'.pdf';
        $pdf->Output($name, 'I');
        //Remember what the user selected - for autoselect on the next print.
        $_SESSION['PAPER_SIZE'] = $psize;
	...
	function updatepincode($pincode) {
		$sql='Update '.TICKET_TABLE.' SET ticketpin='.$pincode.' Where ticketID='.$this->getExtId();
 		db_query($sql);
		@header('Location: tickets.php?id='.$this->getId());
		exit;
		return true;
    ...
    function update($vars, &$errors) {
        ...
        $fields['priorityId'] = array('type'=>'int',    'required'=>0, 'error'=>'Priority required');
...
*/     
        ...
        $sql='UPDATE '.TICKET_TABLE.' SET updated=NOW() '
            .' ,name_title='.db_input(Format::striptags($vars['name_title']))
			.' ,name='.db_input(Format::striptags($vars['name']))
			.' ,applicant_address='.db_input($vars['applicant_address'])
			.' ,nic='.db_input($vars['nic'])
			.' ,phone='.db_input($vars['phone']?$vars['phone']:'')
			.' ,subject='.db_input(Format::striptags($vars['subject']))
            .' ,email='.db_input($vars['email'])
			.' ,district='.db_input($vars['district'])
			.' ,tehsil='.db_input($vars['tehsil'])
			.' ,AgencyTehsil_ID='.db_input($vars['agency_tehsils'])	
			.' ,subject='.db_input(Format::striptags($vars['subject']))
        ...
        if(!db_query($sql) || !db_affected_rows())
    {
			...
			elseif($vars['isquery']==0){
				$deptId=$vars['deptId'];
				if($deptId==2){
				$sql_dept_details = "Select * from sdms_ticket_capital_markets where `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				$sql_capital_markets="UPDATE `sdms_ticket_capital_markets` SET `cm_type` = '".$_POST['cm_type']."', `cm_broker_title` = '".$_POST['cm_broker_title']."', `cm_broker_agent` = '".$_POST['cm_broker_agent']."', `cm_folio_no` = '".$_POST['cm_folio_no']."', `cm_cdc_ac_no` = '".$_POST['cm_cdc_ac_no']."', `cm_no_of_shares` =  '".$_POST['cm_no_of_shares']."' WHERE `complaint_id` = '".$this->getId()."'";
				db_query($sql_capital_markets);
				}else{
				$sql_capital_markets="INSERT INTO `sdms_ticket_capital_markets` ( `cm_type`,`cm_broker_title`,`cm_broker_agent`,`cm_folio_no`, `cm_cdc_ac_no`, `cm_no_of_shares`, `complaint_id`) VALUES ( '".$_POST['cm_type']."','".$_POST['cm_broker_title']."','".$_POST['cm_broker_agent']."','".$_POST['cm_folio_no']."', '".$_POST['cm_cdc_ac_no']."', '".$_POST['cm_no_of_shares']."', '".$this->getId()."')";
				db_query($sql_capital_markets);
				}
				}
				else if($deptId==3){
				$sql_dept_details = "Select * from sdms_ticket_insurance where `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				$sql_ticket_insurance = "Update `sdms_ticket_insurance` SET `i_type`='".$_POST['i_type']."',`i_broker_title`='".$_POST['i_broker_title']."', `i_broker_agent`= '".$_POST['i_broker_agent']."',`i_policy_no`= '".$_POST['i_policy_no']."',  `i_sum_assured` ='".$_POST['i_sum_assured']."',`i_claim_amount`='".$_POST['i_claim_amount']."',`i_folio_no`='".$_POST['i_folio_no']."',`i_no_of_shares`='".$_POST['i_no_of_shares']."' Where `complaint_id`= '".$this->getId()."'";
				db_query($sql_ticket_insurance);
				}else{
				$sql_ticket_insurance = "INSERT INTO `sdms_ticket_insurance` (`i_type`, `i_broker_title`, `i_broker_agent`, `i_policy_no`, `i_sum_assured`, `i_claim_amount`, `i_folio_no`, `i_no_of_shares`, `complaint_id`) VALUES ( '".$_POST['i_type']."', '".$_POST['i_broker_title']."', '".$_POST['i_broker_agent']."', '".$_POST['i_policy_no']."', '".$_POST['i_sum_assured']."', '".$_POST['i_claim_amount']."', '".$_POST['i_folio_no']."', '".$_POST['i_no_of_shares']."', '".$this->getId()."')";
				db_query($sql_ticket_insurance);
				}
				
				}
				else if($deptId==4){
				$sql_dept_details = "Select * from sdms_ticket_scd where `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				$sql_ticket_scd = "Update `sdms_ticket_scd` SET `scd_type` ='".$_POST['scd_type']."', `scd_broker_title`='".$_POST['scd_broker_title']."',`reit_scheme`='".$_POST['reit_scheme']."',`modaraba_fund`='".$_POST['modaraba_fund']."',`mutual_fund`='".$_POST['mutual_fund']."',`pension_fund`='".$_POST['pension_fund']."', `scd_folio_no`='".$_POST['scd_folio_no']."', `scd_cdc_ac_no`='".$_POST['scd_cdc_ac_no']."', `scd_no_of_units`= '".$_POST['scd_no_of_units']."' WHERE `complaint_id` = '".$this->getId()."'";
				db_query($sql_ticket_scd);
				}else{
				$sql_ticket_scd = "INSERT INTO `sdms_ticket_scd` ( `scd_type`, `scd_broker_title`,`reit_scheme`,`modaraba_fund`,`mutual_fund`,`pension_fund`, `scd_folio_no`, `scd_cdc_ac_no`, `scd_no_of_units`,`complaint_id`) VALUES ( '".$_POST['scd_type']."', '".$_POST['scd_broker_title']."', '".$_POST['reit_scheme']."', '".$_POST['modaraba_fund']."', '".$_POST['mutual_fund']."', '".$_POST['pension_fund']."', '".$_POST['scd_folio_no']."', '".$_POST['scd_cdc_ac_no']."', '".$_POST['scd_no_of_units']."', '".$this->getId()."')";
				db_query($sql_ticket_scd);
				}
				...
				$sql_dept_details = "Select * from sdms_ticket_e_services where  `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				$sql_e_services ="Update `sdms_ticket_e_services` SET `e_company_title`='".addslashes($_POST['e_company_title'])."', `e_registration_office`='".$_POST['e_registration_office']."', `e_process_name`='".$_POST['e_process_name']."',`e_user_id`='".$_POST['e_user_id']."' WHERE `complaint_id` = '".$this->getId()."'";
				db_query($sql_e_services);
				}else{
				$sql_e_services ="INSERT INTO `sdms_ticket_e_services` ( `e_company_title`, `e_registration_office`, `e_process_name`,`e_user_id`, `complaint_id`) VALUES ('".addslashes($_POST['e_company_title'])."', '".$_POST['e_registration_office']."', '".$_POST['e_process_name']."','".$_POST['e_user_id']."', '".$this->getId()."')";
				db_query($sql_e_services);
				}
				}
				else if($deptId==6){
				
				$sql_dept_details = "Select * from sdms_ticket_cr where  `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				
				$sql_c_and_c="Update `sdms_ticket_cr` SET `cr_status`='".$_POST['cr_status']."', `cr_company_title`='".addslashes($_POST['cr_company_title'])."',`cr_unregistered_entity`='".$_POST['cr_unregistered_entity']."', `cr_folio_no`='".$_POST['cr_folio_no']."', `cr_no_of_shares`='".$_POST['cr_no_of_shares']."',`cr_cro`='".$_POST['cr_cro']."',`cr_process_name`='".$_POST['cr_process_name']."' WHERE `complaint_id`= '".$this->getId()."'";
				db_query($sql_c_and_c);	
				}else{
				$sql_c_and_c="INSERT INTO `sdms_ticket_cr` (`cr_status`, `cr_company_title`,`cr_unregistered_entity`,`cr_folio_no`, `cr_no_of_shares`,`cr_cro`,`cr_process_name`, `complaint_id`) VALUES ( '".$_POST['cr_status']."', '".addslashes($_POST['cr_company_title'])."','".$_POST['cr_unregistered_entity']."', '".$_POST['cr_folio_no']."', '".$_POST['cr_no_of_shares']."','".$_POST['cr_cro']."','".$_POST['cr_process_name']."', '".$this->getId()."')";
				db_query($sql_c_and_c);						
				}
				
				}
				else if($deptId==18){
				$sql_dept_details = "Select * from sdms_ticket_cs where `complaint_id` = '".$this->getId()."'";
				$res_dept_details = mysql_query($sql_dept_details);
				if(mysql_num_rows($res_dept_details) > 0){
				$sql_againts_cro="UPDATE `sdms_ticket_cs` SET `cs_company_title` ='".addslashes($_POST['cs_company_title'])."' , `cs_cdc_ac_no` ='".$_POST['cs_cdc_ac_no']."' , `cs_folio_no` ='".$_POST['cs_folio_no']."' WHERE `complaint_id` = '".$this->getId()."'";
				db_query($sql_againts_cro);
				}else{
				$sql_againts_cro="INSERT INTO `sdms_ticket_cs` (`cs_company_title` , `cs_cdc_ac_no` , `cs_folio_no`,`complaint_id`) VALUES ('".addslashes($_POST['cs_company_title'])."','".$_POST['cs_cdc_ac_no']."','".$_POST['cs_folio_no']."', '".$this->getId()."')";
				db_query($sql_againts_cro);
				}
				}
		...
		$sql_status='Select complaint_status from  sdms_ticket where ticket_id='.db_input($this->getId());
		$sql_res=mysql_query($sql_status);
		$sql_row=mysql_fetch_array($sql_res);	
		$updated_status = $sql_row['complaint_status'];
		if(!$vars['note'])
            $vars['note']=sprintf('Complaint Updated by %s', $thisstaff->getName());
        $this->logNote('Complaint Updated', $vars['note'],$updated_status, $thisstaff);
        $this->reload();
   ...
   /*============== Static functions. Use Complaint::function(params); ==================*/
...
//echo $id.'<br>';
//$ticket= new Complaint($id);
//echo $ticket;exit;
        return ($id && is_numeric($id) && ($ticket= new Ticket($id)) && $ticket->getId()==$id&& $ticket->getThread())?$ticket:null;
    }
          ...
          $sql='SELECT 
			...
			.' LEFT JOIN '.TICKET_TABLE.' open
				ON (open.ticket_id=ticket.ticket_id
						...
						AND  open.isaccepted=1
						'.((!($cfg->showAssignedTickets() || $staff->showAssignedTickets()))? ' AND open.staff_id=0 ':'').')';
			
			/*	AND open.isanswered=0*/					
            $sql .=' LEFT JOIN '.TICKET_TABLE.' answered
                ON (answered.ticket_id=ticket.ticket_id
                        AND answered.status=\'open\'
                        AND answered.isanswered=1
						AND  answered.isaccepted=1) '
						
            .' LEFT JOIN '.TICKET_TABLE.' overdue
                ON (overdue.ticket_id=ticket.ticket_id
                        AND overdue.status=\'open\'
                        AND overdue.isoverdue=1
						AND  overdue.isaccepted=1) '

           .' LEFT JOIN '.TICKET_TABLE.' rplytouser
                ON (rplytouser.ticket_id=ticket.ticket_id
                        AND rplytouser.status=\'open\'
                        AND rplytouser.ticket_id IN (SELECT ticket_id FROM '.TICKET_THREAD_TABLE.' n
WHERE n.title = \'User Reply\' AND n.id=(SELECT MAX(id) FROM '.TICKET_THREAD_TABLE.' WHERE ticket_id=n.ticket_id) ) ) '
                        
						
			.' LEFT JOIN '.TICKET_TABLE.' deny
                ON (deny.ticket_id=ticket.ticket_id
                        AND deny.status=\'open\'
						AND deny.isquery=0
						AND deny.isrejected=1
						AND  deny.isaccepted=0) '			
						
		    .' LEFT JOIN '.TICKET_TABLE.' lodged
                ON (lodged.ticket_id=ticket.ticket_id
                        AND lodged.status=\'open\'
                        AND lodged.isaccepted=0 
				AND lodged.staff_id='.db_input($staff->getId()).')'
	...
	{
		$sql .=" OR ticket.ticket_id IN (SELECT DISTINCT(`ticket_id`) as ticket_id FROM ".TICKET_EVENT_TABLE." WHERE staff_id='".db_input($staff->getId())." 
	       ' AND `state` = 'assigned')  AND ticket.dept_id= ".$staff->getDeptId()."";
		}
       ...
       */
		...
		$sql.=' AND ticket.isquery=0';
	//echo $sql;exit;
		return db_fetch_array(db_query($sql));
    }

    function getQueryStats($staff) {
        global $cfg;
        /* Unknown or invalid staff */
        if(!$staff || (!is_object($staff) && !($staff=Staff::lookup($staff))) || !$staff->isStaff() || $cfg->getDBVersion())
            return null;
	...
	}else
	{
       ...
       */
        ...
        if(!$cfg || !($cfg->showAssignedTickets() || $staff->showAssignedTickets()))
		    ...
		    $sql.=' AND ticket.isquery=1';	
			//echo $sql;exit;
		return db_fetch_array(db_query($sql));
    }

	function getTotalTickets($nic) {
        if(!$nic)
        	return null;
        $sql='SELECT count(ticket_id) as total from sdms_ticket '
            .' WHERE nic='.db_input($nic);
        
		$res=db_query($sql);
        $this->ht2 = db_fetch_array($res);
		$t_cnic_club   = $this->ht2['total'];
		return $t_cnic_club ;
    }
     ...
     */
       function create($vars, &$errors, $origin, $autorespond=true, $alertstaff=true) {
		...
		/************************Minde Code*****************************/
		  ...
		  $pincode = substr(round($extId*7896/100),0,4);
		 if($vars['email']=="")
		$vars['email'] = $vars['nic']."@novalidemail.pk";
       ...
       /* if ($vars['email']  && Validator::is_email($vars['email'])) {
            ...
            if(TicketFilter::isBanned($vars['email'])) {
                $errors['err']='Complaint denied. Error #403';
                $errors['errno'] = 403;
                $ost->logWarning('Complaint denied', 'Banned email - '.$vars['email']);
                return 0;
            }
			
			//print_r($vars);exit;		  
		    //echo $cfg->getMaxOpenComplaints() && ;exit;
            //Make sure the open ticket limit hasn't been reached. (LOOP CONTROL)
           if( strcasecmp($origin,'staff') && ($client = Client::lookupByEmail($vars['email'])) && ($openTickets = $client->getNumOpenTickets()) && ($openTickets >= $cfg->getMaxOpenTickets())){
				$errors['err']="You've reached the maximum open Complaints allowed.";
                $ost->logWarning('Complaint denied -'.$vars['email'],
                sprintf('Max open Complaint (%d) reached for %s ',
		...
		*/	 
		...
		if($ticket_filter && ($filter=$ticket_filter->shouldReject())) {
            $errors['err']='Complaint denied. Error #403';
            $errors['errno'] = 403;
            $ost->logWarning('Complaint denied',
                    sprintf('Complaint rejected ( %s) by filter "%s"',
        ...
        $fields['name']     = array('type'=>'string',   'required'=>1, 'error'=>'Name required');
        if($vars['identify']==2)
        $fields['name']     = array('type'=>'string',   'required'=>0, 'error'=>'Name required');
		$fields['email']    = array('type'=>'email',    'required'=>0, 'error'=>'Valid email required');
        $fields['subject']  = array('type'=>'string',   'required'=>1, 'error'=>'Subject required');
        ...
        //Any error above is secpl.
		
        if($errors)  return 0;
		
        # Perform Complaint filter actions on the new Complaint arguments
        ...
        if(isset($vars['topicId']) && ($topic=Topic::lookup($vars['topicId']))) { //Complaint created via web by user/or staff
		...
		elseif($vars['emailId'] && !$vars['deptId'] && ($email=Email::lookup($vars['emailId']))) { //Emailed Complaints
		...
		$district =   $vars['district'];
          ...
          $sql='INSERT INTO '.TICKET_TABLE.' SET created=NOW() '
			...
			.' ,applicant_address='.db_input($vars['applicant_address'])
			.' ,nic='.db_input($vars['nic'])
			.' ,gender='.db_input($vars['gender'])
			.' ,occupation="'.$vars['occupation'].'"'
            .' ,phone='.db_input($vars['phone']?$vars['phone']:'')
		    .' ,landline_number='.db_input($vars['phone']?$vars['landline_number']:'')
			.' ,email='.db_input($vars['email'])
			.' ,alternate_email='.db_input($vars['alternate_email'])
			.' ,applicant_location='.db_input($vars['applicant_location'])
			.' ,district='.db_input($district)
			.' ,service_id='.db_input($vars['service'])
			.' ,tehsil='.db_input($vars['tehsil'])	
			.' ,AgencyTehsil_ID='.db_input($vars['agency_tehsils'])			
			.' ,pre_complaint='.db_input($vars['pre_complaint'])		
			.' ,source='.db_input($source)
			.' ,subject='.db_input(Format::striptags($vars['subject']))
			.' ,priority_id='.db_input($priorityId)
			.' ,topic_id='.db_input($topicId)
			.' ,age='.db_input($vars['age'])	
			.' ,isquery='.db_input($vars['isquery'])
			.' ,address='.db_input($vars['address'])
			.' ,lastmessage=NOW()'
            .' ,ticketID='.db_input($extId)
			.' ,ticketpin='.db_input($pincode)
		...
		return null;
			...
			if($vars['isquery']==1){
					if($vars['query_rsolved']==1)
					{
						db_query('UPDATE '.TICKET_TABLE.' SET isaccepted=1 WHERE ticket_id='.$id.' LIMIT 1');
					}
			}
			elseif($vars['isquery']==0){
				$img_src ='';
			if($_FILES['cm_evidence']['name']!='' && $deptId==2){
			$target_path = "scp/upload/"; 
			$rand=rand(00000,999999);
			$target_path = $target_path . basename($rand.$_FILES['cm_evidence']['name']);
			$img_src = basename($rand.$_FILES['cm_evidence']['name']);
			$tmp = $_FILES['cm_evidence']['tmp_name'];
			move_uploaded_file($tmp,$target_path);
			}
			
			if($_FILES['i_evidence']['name']!='' && $deptId==3){
			$target_path = "scp/upload/"; 
			$rand=rand(00000,999999);
			$target_path = $target_path . basename($rand.$_FILES['i_evidence']['name']);
			$img_src = basename($rand.$_FILES['i_evidence']['name']);
			$tmp = $_FILES['i_evidence']['tmp_name'];
			move_uploaded_file($tmp,$target_path);
			}
			
			if($_FILES['scd_evidence']['name']!='' && $deptId==4){
			$target_path = "scp/upload/"; 
			$rand=rand(00000,999999);
			$target_path = $target_path . basename($rand.$_FILES['scd_evidence']['name']);
			$img_src = basename($rand.$_FILES['scd_evidence']['name']);
			$tmp = $_FILES['scd_evidence']['tmp_name'];
			move_uploaded_file($tmp,$target_path);
			}
			
			$sql_evidence="INSERT INTO ".TICKET_EVIDENCE_TABLE." SET created = NOW() , evidence_1='".$vars['evidence_1']."',evidence_2='".$vars['evidence_2']."',evidence_3='".$vars['evidence_3']."', evidence_4='".$vars['evidence_4']."' , evidence_5='".$vars['evidence_5']."' , evidence_6='".$vars['evidence_6']."', evidence_7='".$img_src."',ticket_id='".$id."'";
			db_query($sql_evidence);
						
			if($deptId==2){
			$sql_capital_markets="INSERT INTO `sdms_ticket_capital_markets` ( `cm_type`,`cm_broker_title`,`cm_broker_agent`,`cm_folio_no`, `cm_cdc_ac_no`, `cm_no_of_shares`, `complaint_id`) VALUES ( '".$_POST['cm_type']."','".$_POST['cm_broker_title']."','".$_POST['cm_broker_agent']."','".$_POST['cm_folio_no']."', '".$_POST['cm_cdc_ac_no']."', '".$_POST['cm_no_of_shares']."', '".$id."')";
			db_query($sql_capital_markets);
			}
			else if($deptId==3){
				
			$sql_ticket_insurance = "INSERT INTO `sdms_ticket_insurance` (`i_type`, `i_broker_title`, `i_broker_agent`, `i_policy_no`, `i_sum_assured`, `i_claim_amount`, `i_folio_no`, `i_no_of_shares`, `complaint_id`) VALUES ( '".$_POST['i_type']."', '".$_POST['i_broker_title']."', '".$_POST['i_broker_agent']."', '".$_POST['i_policy_no']."', '".$_POST['i_sum_assured']."', '".$_POST['i_claim_amount']."', '".$_POST['i_folio_no']."', '".$_POST['i_no_of_shares']."', '".$id."')";
			db_query($sql_ticket_insurance);
			}
			else if($deptId==4){
			$sql_ticket_scd = "INSERT INTO `sdms_ticket_scd` ( `scd_type`, `scd_broker_title`,`reit_scheme`,`modaraba_fund`,`mutual_fund`,`pension_fund`, `scd_folio_no`, `scd_cdc_ac_no`, `scd_no_of_units`,`complaint_id`) VALUES ( '".$_POST['scd_type']."', '".$_POST['scd_broker_title']."', '".$_POST['reit_scheme']."', '".$_POST['modaraba_fund']."', '".$_POST['mutual_fund']."', '".$_POST['pension_fund']."', '".$_POST['scd_folio_no']."', '".$_POST['scd_cdc_ac_no']."', '".$_POST['scd_no_of_units']."', '".$id."')";
			...
			$sql_e_services ="INSERT INTO `sdms_ticket_e_services` ( `e_company_title`, `e_registration_office`, `e_process_name`,`e_user_id`, `complaint_id`) VALUES ('".addslashes($_POST['e_company_title'])."', '".$_POST['e_registration_office']."', '".$_POST['e_process_name']."','".$_POST['e_user_id']."', '".$id."')";
			db_query($sql_e_services);
			}
			else if($deptId==6){
			$sql_c_and_c="INSERT INTO `sdms_ticket_cr` (`cr_status`, `cr_company_title`,`cr_unregistered_entity`, `cr_folio_no`, `cr_no_of_shares`,`cr_cro`,`cr_process_name`, `complaint_id`) VALUES ( '".$_POST['cr_status']."', '".addslashes($_POST['cr_company_title'])."','".$_POST['cr_unregistered_entity']."', '".$_POST['cr_folio_no']."', '".$_POST['cr_no_of_shares']."','".$_POST['cr_cro']."','".$_POST['cr_process_name']."', '".$id."')";
		 ...
		 $sql_againts_cro="INSERT INTO `sdms_ticket_cs` (`cs_company_title` , `cs_cdc_ac_no` , `cs_folio_no`,`complaint_id`) VALUES ('".addslashes($_POST['cs_company_title'])."','".$_POST['cs_cdc_ac_no']."','".$_POST['cs_folio_no']."', '".$id."')";
		...
		if($vars['isquery']==1){
		$message .= 'We have received your query and forwarded it to the relevant department. You can track your query by the following query number '.$extId.' and pincode is '.$pincode.'.  You should expect a response shortly.<br>';
		}else{
		$message .= 'Your Complaint titled '.db_input(Format::striptags($vars['subject'])).' is being assessed by SECP. You can track your complaint by using the following complaint number '.$extId.' and pincode is '.$pincode.'.<br>';
		}
		$message .= "Regards,<br>SECP Service Desk<br>Toll Free: 080088008";
		//Please do not respond to this sms
		if($vars['isquery']==1){
		//$message .="Email:  queries@secp.gov.pk<br>Please do not respond to this sms";
		$from_email = 'queries@secp.gov.pk';
		}else{
		//$message .="Email:  complaints@secp.gov.pk<br>Please do not respond to this sms";
			$from_email = 'complaints@secp.gov.pk';
		}
		if($vars['query_rsolved']==0)
					{
...
//https://connect.jazzcmt.com
		...
		curl_setopt_array($curl, array(
		CURLOPT_RETURNTRANSFER => 1,
		/*CURLOPT_URL => 'https://connect.jazzcmt.com/sendsms_url.html?Username=03028504208&Password=123.123&From=8181&To='.$vars['phone'].'&Message='.$message.'',*/
        CURLOPT_URL => 'http://smsctp1.eocean.us:24555/api?action=sendmessage&username=SECP&password=P@87654&recipient='.$vars['phone'].'&originator=8181&messagedata='.$message.'',
	 ...
	 /*	$to = $vars['email'];
		...
		$mail_me= mail($to, $subject, $email_message, $headers);*/
		}
			
		// Prepare data for POST request
		/*$sql="INSERT INTO `sdms_sms_logs`(`ticketID`, `sms_date`, `send_to`, `sms_text`, `staff_id`) VALUES ('".$extId."','NOW()','".$selectednums."','".$message."','".$_SESSION['Staff_id']."')";
			db_query($sql);	*/
	...
	function chage_pincode($pincode, &$errors, $origin, $autorespond=true, $alertstaff=true) {
        global $ost, $cfg, $thisclient, $_FILES;		
        //We are ready son...hold on to the rails.
        //echo 'ID is :'.$_SESSION['_client']['key'];
		  $sql='Update '.TICKET_TABLE.' SET ticketpin='.$pincode.' Where ticketID='.$_SESSION['_client']['key'];
 			db_query($sql);
		// Sms Code 
		$uname = 'carl@telewin.co.uk';
		$pword = 'icevalley';
		$from = 'HRD-PHC';
		$selectednums = '+92'.substr($vars['phone'],1);
		$message = 'Your Pincode has been successfully Change. UCN:'.$extId.' PINCODE:'.$pincode.'. Please contact 091-9213632 for further details.';
		$message = urlencode($message);
	...
	//Open will call the Create ftn to create new complaint
    ...
    function open($vars, &$errors) {
        ...
        $dept_assign_staff = $row_get_focal['staff_id'];
        if($vars['assignId'] == 0 || $vars['assignId']!=$dept_assign_staff){
            $vars['assignId'] = $dept_assign_staff;
        }


		//echo $vars['source'];exit;
        if($vars['source'] && !in_array($vars['source'],array('At Counter','From Chief Justice','By Email','Through Call','By Post','By Fax','Online Complaint','SECP Chatbot')))
	...
	if($vars['assignId'] && $thisstaff->canAssignTickets()) { //Assign ticket to staff or team.
            $ticket->assign($vars['assignId'], $vars['note'],'7');
        } elseif($vars['note']) { //Not assigned...save optional note if any
         
		    $ticket->logNote('New Complaint', $vars['note'],$ticket->getStatus(), $thisstaff, false);
        } else { //Not assignment and no internal note - log activity
         	$ticket->logActivity('New Complaint by Staff','Complaint created by staff -'.$thisstaff->getName());
        }
	...
	function accepted($vars, &$errors) {
		$id = $vars['id'];
		 $ipaddress = $_SERVER['REMOTE_ADDR'];
		$sql='Update '.TICKET_TABLE.' SET updated=NOW() '
            .' ,isaccepted='.db_input(1)	
			.' ,isrejected=0'
			.' WHERE ticket_id='.db_input($id).'';
        if(!db_query($sql) || !($ticket = Ticket::lookup($id)))
		{
		return null;
		}else{
		$staff=Staff::lookup($_SESSION['_staff']['ID']);
		$t_type = 'T';
		$t_title = $staff->getName().' accepted the complaint';
		$t_body = 'Complaint is accepted by '.$staff->getName();
		$staffId = $_SESSION['_staff']['ID'];
		$poster = $staff->getName();
		$default_stauts ='7';
		$source = '';
        $sql_thread=' INSERT INTO '.TICKET_THREAD_TABLE.' SET created=NOW() '
            .' ,thread_type='.db_input($t_type)
            .' ,ticket_id='.db_input($id)
            .' ,title='.db_input(Format::sanitize($t_title, true))
            .' ,body='.db_input(Format::sanitize($t_body, true))
            .' ,staff_id='.db_input($staffId)
            .' ,poster='.db_input($poster)
			.' ,complaint_status='.db_input($default_stauts)
            .' ,source='.db_input($source);
            $sql_thread.=' ,ip_address='.db_input($ipaddress);
        db_query($sql_thread);	
		
	
		$sql_complaint_info ="Select * from sdms_ticket where ticket_id='".db_input($id)."' ORDER By ticket_id limit 0,1";
		$res_complaint_info = mysql_query($sql_complaint_info);
		$row_complaint_info = mysql_fetch_array($res_complaint_info);
		
		$sql_dept_info ="Select * from sdms_department where dept_id=".$row_complaint_info['dept_id']."";
		$res_dept_info = mysql_query($sql_dept_info);
		$row_dept_info = mysql_fetch_array($res_dept_info);
		
	

if($row_complaint_info['isquery']=='0'){
	$ticket_type = 'Complaint';
	$subject = "Complaint received for handling by relevant department of SECP";
}else{
	$ticket_type = 'Query';
	$subject = "Query received for handling by relevant department of SECP";
}
		// Sms Code 
        //  ".$row_dept_info['dept_name']." department
		$message = "Dear Sir/Miss,<br>Your ".$ticket_type." titled ".$row_complaint_info['subject']." has been accepted for handling by the SECP. You can track your complaint by using the following complaint number ".$row_complaint_info['ticketID']." and pincode is ".$row_complaint_info['ticketpin'].".<br>Regards,<br>SECP Service Desk<br>Toll Free: 080088008";
		$email_message = $message; 		
		$message = urlencode($message);
		$curl = curl_init();
		curl_setopt_array($curl, array(
		CURLOPT_RETURNTRANSFER => 1,
		/*CURLOPT_URL => 'https://connect.jazzcmt.com/sendsms_url.html?Username=03028504208&Password=123.123&From=8181&To='.$row_complaint_info['phone'].'&Message='.$message.'',*/
        CURLOPT_URL => 'http://smsctp1.eocean.us:24555/api?action=sendmessage&username=SECP&password=P@87654&recipient='.$row_complaint_info['phone'].'&originator=8181&messagedata='.$message.'',
		CURLOPT_USERAGENT => 'SDMS SECP'
		));
		$resp = curl_exec($curl);
		curl_close($curl);
		
		 $sql_thread1=' INSERT INTO '.TICKET_THREAD_TABLE.' SET created=NOW() '
            .' ,thread_type='.db_input($t_type)
            .' ,ticket_id='.db_input($id)
            .' ,title='.db_input(Format::sanitize($subject, true))
            .' ,body='.db_input(Format::sanitize($email_message, true))
            .' ,staff_id='.db_input($staffId)
            .' ,poster='.db_input($poster)
			.' ,complaint_status='.db_input($default_stauts)
            .' ,source='.db_input($source);
            $sql_thread.=' ,ip_address='.db_input($ipaddress);
        db_query($sql_thread1);	
		
		
		if($row_complaint_info['email']!='')
		{	
		$from_email = 'alerts.servicedesk@secp.gov.pk';
		$to = $row_complaint_info['email'];
		$headers = "MIME-Version: 1.0" . "\r\n";
		$headers .= "Content-type:text/html;charset=iso-8859-1" . "\r\n";
		$headers .= 'From: <'.$from_email.'>' . "\r\n";
		$mail_me= mail($to, $subject, $email_message, $headers);
		...
		if($row_complaint_info['dept_id']=='2' && $row_complaint_info['isquery']=='0')
		{
		$sql_complaint_details="Select * from sdms_ticket_thread where ticket_id='".db_input($id)."' ORDER By id limit 0,1";
		$res_complaint_details=mysql_query($sql_complaint_details);
		$row_complaint_details=mysql_fetch_array($res_complaint_details);
		$from_email = 'alerts.servicedesk@secp.gov.pk';
		
		$to = 'shahid.nasim@secp.gov.pk';
		//$to = 'mr.elahi.ehsan@gmail.com';
		$subject = "Capital Markets POC accepted the complaint number ".$row_complaint_info['ticketID']." ";
		$email_message = "Dear Sir,<br>Complaints # ".$row_complaint_info['ticketID']." with titled ".$row_complaint_info['subject']." has been accepted for handling by the ".$row_dept_info['dept_name']." department.<br />Coplaint Details is below<br />".$row_complaint_details['body']."<br>Regards,<br>SECP Service Desk<br>Toll Free: 080088008";
		$headers = "MIME-Version: 1.0" . "\r\n";
		$headers .= "Content-type:text/html;charset=iso-8859-1" . "\r\n";
		$headers .= 'From: <'.$from_email.'>' . "\r\n";
		$headers .= 'Cc: ahsan@crazenators.com\r\n';
		$mail_me= mail($to, $subject, $email_message, $headers);
		}
		
		//working for new thread
    ...
    function deny($vars, &$errors) {
		...
		$staff=Staff::lookup($_SESSION['_staff']['ID']);
		$t_type = 'T';
		$t_title = $staff->getName().' deny the complaint';
		$t_body = $vars['note'];
		$staffId = $_SESSION['_staff']['ID'];
		$poster = $staff->getName();
		$default_stauts ='7';
	...
	function set_category($vars, &$errors) {
		$id = $vars['id'];
		 $ipaddress = $_SERVER['REMOTE_ADDR'];
		$sql='Update '.TICKET_TABLE.' SET updated=NOW() '	
			.' ,topic_id='.db_input($vars['topicId'])	
        	.' WHERE ticket_id='.db_input($id).'';
			//.' ,ishandel_reassign=0'.db_input($vars['topicId'])	
        if(!db_query($sql) || !($ticket = Ticket::lookup($id)))
		{
		return null;
		}else{
		$staff=Staff::lookup($_SESSION['_staff']['ID']);
		$t_type = 'C';
		$t_title = $staff->getName().' added the complaint category';
		$t_body = 'Complaint is updated by '.$staff->getName();
		$staffId = $_SESSION['_staff']['ID'];
		$poster = $staff->getName();
		$default_stauts = $ticket->complaint_status_title();
		$source = '';
        $sql_thread=' INSERT INTO '.TICKET_THREAD_TABLE.' SET created=NOW() '
            .' ,thread_type='.db_input($t_type)
            .' ,ticket_id='.db_input($id)
        ...
        if(($res=db_query($sql)) && db_num_rows($res)) {
            while(list($id)=db_fetch_row($res)) {
                if(($ticket=Ticket::lookup($id)) && $ticket->markOverdue())
                    $ticket->logActivity('Complaint Marked Overdue', 'Complaint flagged as overdue by the system.');
            }
        } else {
