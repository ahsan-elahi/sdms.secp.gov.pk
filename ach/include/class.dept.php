...
class Dept {
    ...
    function getDepartments( $criteria=null) {
        ...
        $depts=array();
        $sql='SELECT dept_id, dept_name,islocation FROM '.DEPT_TABLE.' WHERE dept_p_id=0 ';
	 ...
	 function getuserDepartments( $criteria=null) {
        
        $depts=array();
        $sql='SELECT dept_id, dept_name,islocation FROM '.DEPT_TABLE.'';
        if($criteria['publiconly'])
            $sql.=' AND  ispublic=1';
    ...
    function getPublicDepartments() {
		...
		//-----------------------------------------------------------------------------------------------------------------------------------------//
	      function getSubPublicCategory($complaint_id) {
        return self::getSubCategory(true,$complaint_id);
    }
		
		
		function getSubCategory($publicOnly=false,$complaint_id) {
        $sub_depart=array();
		if($complaint_id!='')
		{
		$sql_sub_depart='SELECT ht.dept_id, ht.dept_name as name '
            .' FROM '.DEPT_TABLE. ' ht '
            .' WHERE ht.dept_p_id="'.$complaint_id.'" ';
		

        if($publicOnly)
            $sql_sub_depart.=' AND ht.ispublic=1';


        $sql_sub_depart.=' ORDER BY sort_order ';


        if(($res=db_query($sql_sub_depart)) && db_num_rows($res))
            while(list($id, $name)=db_fetch_row($res))
                $sub_depart[$id]=$name;
		}
        return $sub_depart;
    ...
    function save($id, $vars, &$errors) {
        ...
        $sql='SET updated=NOW() '
		    .',dept_p_id='.db_input($vars['dept_p_id'])
            .' ,ispublic='.db_input($vars['ispublic'])
			.' ,islocation='.db_input($vars['islocation'])
