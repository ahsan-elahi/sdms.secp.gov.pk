...
class Topic {
    ...
    function getHelpTopics($deptID,$isnature,$publicOnly=false) {
	// topic_id 	topic_pid 
        $topics=array();
        $sql='SELECT ht.topic_id, CONCAT_WS(" / ", ht2.topic, ht.topic) as name '
            .' FROM '.TOPIC_TABLE. ' ht '
            .' LEFT JOIN '.TOPIC_TABLE.' ht2 ON(ht2.topic_id=ht.topic_pid) '
            .' WHERE ht.isactive=1 AND ht.topic_pid=0';
		
		/*$sql='SELECT ht.topic_id, ht.topic as name '
            .' FROM '.TOPIC_TABLE. ' ht '
            .' WHERE ht.isactive=1 AND ht.topic_pid=0';*/
		...
		if($deptID)
            $sql.=' AND ht.dept_id='.$deptID.'';
			
		if(isset($isnature))
            $sql.=' AND ht.isnature='.$isnature.'';	
    ...
    function getPublicHelpTopics($deptID,$isnature) {
        return self::getHelpTopics($deptID,$isnature,true);
    }
	
	
	//-----------------------------------------------------------------------------------------------------------------------------------------//
	      function getSubPublicHelpTopics($cat_id) {
        return self::getSubHelpTopics(true,$cat_id);
    }
		
		
		function getSubHelpTopics($publicOnly=false,$cat_id) {
	// topic_id 	topic_pid 
        $sub_topics=array();
        /*$sql='SELECT ht.topic_id, CONCAT_WS(" / ", ht2.topic, ht.topic) as name '
            .' FROM '.TOPIC_TABLE. ' ht '
		...
		$sql_sub_topics='SELECT ht.topic_id, ht.topic as name '
            .' FROM '.TOPIC_TABLE. ' ht '
            .' WHERE ht.isactive=1 AND ht.topic_pid="'.$cat_id.'" ';

        if($publicOnly)
            $sql_sub_topics.=' AND ht.ispublic=1';

        $sql_sub_topics.=' ORDER BY name';
    ...
    function save($id, $vars, &$errors) {
        ...
        if(!$vars['topic'])
            $errors['topic']='Category Required';
        elseif(strlen($vars['topic'])<3)
            $errors['topic']='Topic is too short. 3 chars minimum';
        ...
        $sql=' updated=NOW() '
			...
			.',isnature='.db_input($vars['isnature'])
        ...
        if($id) {
            ...
            $errors['err']='Unable to update Category. Internal error occurred';
        } else {
            $sql='INSERT INTO '.TOPIC_TABLE.' SET '.$sql.',created=NOW()';
			if(db_query($sql) && ($id=db_insert_id()))
                return $id;
            
            $errors['err']='Unable to create the Category. Internal error';
        }
