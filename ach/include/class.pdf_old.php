<?php define('THIS_DIR', str_replace('\\\\', '/', realpath(dirname(__FILE__))) . '/'); //Include path..
...
class Ticket2PDF extends FPDF
{
    ...
    function _print() {
		...
		//Complaint #0000    Applicant Information
        $this->SetFont('Arial', 'B', 11);
        $this->cMargin = 3;
		$this->SetTextColor(10, 86, 142);
        $this->Cell($w, 7,'Complaint #'.$ticket->getNumber(), 0, 0, 'L');
        $this->SetTextColor(0);
        ...
        $this->SetFont('Arial', 'B', 11);
        $this->SetTextColor(10, 86, 142);
        $this->Cell($w, 7,'Applicant Information', 0, 0, 'L');
        $this->Ln(7);
        $this->cMargin = 3;
        $this->SetTextColor(0);
        $this->SetDrawColor(220, 220, 220);
        ...
        $this->SetFont('');
        $this->Cell($c, 7,  $ticket->complaint_status(), 1, 0, 'L', true);
		...
		$this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, $ticket->getRelation_Title(), 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7,  $ticket->getRelation_Name(), 1, 1, 'L', true);
		
		//Complaint Date and CNIC
		$this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Complaint Date', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7,  Format::db_datetime($ticket->getCreateDate()), 1, 0, 'L', true);
        
		$this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'CNIC', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7, $ticket->getNic(), 1, 1, 'L', true);	
		
		//Category and Gender
		$this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Category', 1, 0, 'L', true);
        $this->SetFont('');
		$topic=explode('/',$ticket->getHelpTopic());
        $this->Cell($c, 7, $topic[1], 1, 0, 'L', true);
        
		$this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Gender', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7, $ticket->getGender(), 1, 1, 'L', true);		
		...
		$this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Cell Phone', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7, $ticket->getPhoneNumber(), 1, 1, 'L', true);		
		
		//Null and District
		$this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, '', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7, '', 1, 0, 'L', true);
        
		$this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Agency', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7, $ticket->getDistrict(), 1, 1, 'L', true);		
		
		//Null and Address
		$this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Address', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c + $l + $c, 7, $ticket->getApplicant_Address(), 1, 0, 'L', true);
        
		//$this->SetFont('Arial', 'B', 11);
        //$this->Cell($l, 7, '', 1, 0, 'L', true);
        //$this->SetFont('');
        //$this->MultiCell( $c, 7, '', 1, 0, 'L', true);
