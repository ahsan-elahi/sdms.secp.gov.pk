...
Your query number ".$row_info['ticketID']." has been resolved.  To provide us feedback, please click on the link provided below
<a href='http://172.16.199.184/feedback.php?id=".$row_info['ticket_id']."'>Click Here For Feedback</a>
SDMS SECP";
    ...
    function onOpenLimit($sendNotice=true) {
        ...
        $ost->logWarning('Max. Open Query Limit ('.$this->getEmail().')', $msg);
        ...
        $alert='Max. open Complaints reached for '.$this->getEmail()."\n"
              .'Open Query: '.$client->getNumOpenTickets()."\n"
    ...
    function onAssign($assignee, $comments,$status, $alert=true) {
        ...
        $this->reload();
        $comments = $comments?$comments:'Query assignment';
        $assigner = $thisstaff?$thisstaff:'SYSTEM (Auto Assignment)';
		
        //Log an internal note - no alerts on the internal note.
		
		$this->logNote('Query Assigned to '.$assignee->getName(), $comments,$status, $assigner, false);
   ...
   function assign($assignId, $note,$status, $alert=true) {
	 ...
	 //Query Replay to End User  or reply.
	 ...
	 function transfer($deptId, $comments, $alert = true) {
        ...
        $title='Query transfered from '.$currentDept.' to '.$this->getDeptName();
    ...
    //Insert Internal Notes/Execute on Assign query to Members 
    ...
    function pdfExport($psize='Letter', $notes=false) {
        ...
        $pdf = new Query2PDF($this, $psize, $notes);
        $name='Query-'.$this->getExtId().'.pdf';
        $pdf->Output($name, 'I');
...
*/     
   ...
   /*============== Static functions. Use Query::function(params); ==================*/
...
//$query= new Query($id);
//echo $query;exit;
        return ($id && is_numeric($id) && ($query= new Query($id)) && $query->getId()==$id&& $query->getThread())?$query:null;
    }
     ...
     */
       ...
       /* if ($vars['email']  && Validator::is_email($vars['email'])) {
            ...
            if(TicketFilter::isBanned($vars['email'])) {
                $errors['err']='Query denied. Error #403';
                $errors['errno'] = 403;
                $ost->logWarning('Query denied', 'Banned email - '.$vars['email']);
                return 0;
            }
           ...
           if( strcasecmp($origin,'staff') && ($client = Client::lookupByEmail($vars['email'])) && ($openTickets = $client->getNumOpenTickets()) && ($openTickets >= $cfg->getMaxOpenTickets())){
                ...
                $ost->logWarning('Query denied -'.$vars['email'],
                sprintf('Max open Query (%d) reached for %s ',
		...
		*/	 
        ...
        $query_filter = new TicketFilter($origin, $vars);
        // Make sure email contents should not be rejected
        	
		if($query_filter && ($filter=$query_filter->shouldReject())) {
            $errors['err']='Query denied. Error #403';
            $errors['errno'] = 403;
            $ost->logWarning('Query denied',
                    sprintf('Query rejected ( %s) by filter "%s"',
        ...
        # Perform Query filter actions on the new Query arguments
        if ($query_filter) $query_filter->apply($vars);
        ...
        if(isset($vars['topicId']) && ($topic=Topic::lookup($vars['topicId']))) { //Query created via web by user/or staff
		...
		//echo $sql;exit;
		if(!db_query($sql) || !($id=db_insert_id()) || !($query =Query::lookup($id)))
		{
		return null;
		}else{
			if($vars['isquery']==1){
			...
			$sql_ticket_scd = "INSERT INTO `sdms_ticket_scd` ( `scd_type`, `scd_broker_title`,`reit_scheme`,`modaraba_fund`,`mutual_fund`,`pension_fund`, `scd_broker_agent`, `scd_folio_no`, `scd_cdc_ac_no`, `scd_no_of_units`,`complaint_id`) VALUES ( '".$_POST['scd_type']."', '".$_POST['scd_broker_title']."', '".$_POST['reit_scheme']."', '".$_POST['modaraba_fund']."', '".$_POST['mutual_fund']."', '".$_POST['pension_fund']."', '".$_POST['scd_folio_no']."', '".$_POST['scd_cdc_ac_no']."', '".$_POST['scd_no_of_units']."', '".$id."')";
			db_query($sql_ticket_scd);
			}
			else if($deptId==5){
			$sql_e_services ="INSERT INTO `sdms_ticket_e_services` ( `e_company_title`, `e_registration_office`, `e_process_name`,`e_user_id`, `complaint_id`) VALUES ('".$_POST['e_company_title']."', '".$_POST['e_registration_office']."', '".$_POST['e_process_name']."','".$_POST['e_user_id']."', '".$id."')";
			db_query($sql_e_services);
			}
			else if($deptId==6){
			$sql_c_and_c="INSERT INTO `sdms_ticket_cr` (`cr_status`, `cr_company_title`, `cr_folio_no`, `cr_no_of_shares`,`cr_cro`,`cr_process_name`, `complaint_id`) VALUES ( '".$_POST['cr_status']."', '".$_POST['cr_company_title']."', '".$_POST['cr_folio_no']."', '".$_POST['cr_no_of_shares']."','".$_POST['cr_cro']."','".$_POST['cr_process_name']."', '".$id."')";
			db_query($sql_c_and_c);						
			}
			else if($deptId==18){
			$sql_againts_cro="INSERT INTO `sdms_ticket_cs` (`cs_company_title` , `cs_cdc_ac_no` , `cs_folio_no`,`complaint_id `) VALUES ('".$_POST['cs_company_title']."','".$_POST['cs_cdc_ac_no']."','".$_POST['cs_folio_no']."', '".$id."')";
		...
		$message .= 'Your query has reached us and is currently under review. You can track your query by the following query number '.$extId.' and pincode is '.$pincode.'. You should expect an update in this regard shortly.<br>';
		}
	 ...
	 /*	$to = $vars['email'];
		...
		$dept = $query->getDept();
        //post the message.
        unset($vars['cannedattachments']); //Query::open() might have it set as part of  open & respond.
        $vars['title'] = $vars['subject']; //Use the initial subject as title of the post.
		
		
        $message = $query->postMessage($vars , $origin, false);
			
        // Configure service-level-agreement for this ticket
        $query->selectSLAId($vars['slaId']);
        //Auto assign staff or team - auto assignment based on filter rules.
        if($vars['staffId'] && !$vars['assignId'])
		     $query->assignToStaff($vars['staffId'], 'Auto Assignment');
			 
        if($vars['teamId'] && !$vars['assignId'])
            $query->assignToTeam($vars['teamId'], 'Auto Assignment');
		
        /**********   double check auto-response  ************/
        //Overwrite auto responder if the FROM email is one of the internal emails...loop control.
        if($autorespond && (Email::getIdByEmail($query->getEmail())))
            $autorespond=false;
        ...
        if ($vars['cannedResponseId']
            && $query->postCannedReply($vars['cannedResponseId'], $message->getId(), $autorespond)) {
                $query->markUnAnswered(); //Leave the ticket as unanswred.
                $autorespond = false;
        }
        ...
        //$query->onNewTicket($message, $autorespond, $alertstaff);
        ...
        if($cfg->getMaxOpenTickets()>0
            ...
            $query->onOpenLimit(($autorespond && strcasecmp($origin, 'staff')));
        }
		
        /* Start tracking ticket lifecycle events */
        $query->logEvent('created');
	...
	//Open will call the Create ftn to create new query
    ...
    function open($vars, &$errors) {
        ...
        if($vars['source'] && !in_array($vars['source'],array('At Counter','From Chief Justice','By Email','Through Call','By Post','By Fax','Online Query','SECP Chatbot')))
        ...
        if(!($query=Query::create($vars, $errors, 'staff', false, (!$vars['assignId']))))
            return false;
			

        $vars['msgId']=$query->getLastMsgId();
        // post response - if any
        $response = null;
        if($vars['response'] && $thisstaff->canPostReply()) {
            $vars['response'] = $query->replaceVars($vars['response']);
            if(($response=$query->postReply($vars, $errors, false))) {
                //Only state supported is closed on response
                if(isset($vars['ticket_state']) && $thisstaff->canCloseTickets())
                   $query->setState($vars['ticket_state']);
            }
        }
	...
	if($vars['assignId'] && $thisstaff->canAssignTickets()) { //Assign ticket to staff or team.
            $query->assign($vars['assignId'], $vars['note'],'7');
        } elseif($vars['note']) { //Not assigned...save optional note if any
         
		    $query->logNote('New Query', $vars['note'],$query->getStatus(), $thisstaff, false);
        } else { //Not assignment and no internal note - log activity
         	$query->logActivity('New Query by Staff','Query created by staff -'.$thisstaff->getName());
        }
		
		$query->reload();
        if(!$cfg->notifyONNewStaffTicket() || !isset($vars['alertuser']))
		            return $query; //No alerts.
		...
		$email->send($query->getEmail(), $msg['subj'], $msg['body'], $attachments);
		}
        return $query;
    }
	...
	function accepted($vars, &$errors) {
        ...
        if(!db_query($sql) || !($query = Query::lookup($id)))
		{
		return null;
		}else{
		$staff=Staff::lookup($_SESSION['_staff']['ID']);
		$t_type = 'T';
		$t_title = $staff->getName().' accepted the query';
		$t_body = 'Query is accepted by '.$staff->getName();
...
if($row_complaint_info['isquery']=='0'){
	...
	$subject = "Query received for handling by relevant department of SECP";
		...
		$message = "Dear Sir/Miss,<br>Your ".$query_type." titled ".$row_complaint_info['subject']." has been accepted for handling by the SECP. You can track your query by using the following query number ".$row_complaint_info['ticketID']." and pincode is ".$row_complaint_info['ticketpin'].".<br>Regards,<br>SECP Service Desk<br>Toll Free: 080088008<br>Email: queries@secp.gov.pk<br>complaints@secp.gov.pk<br>Please do not respond to this sms";
		$email_message = $message; 		
		$message = urlencode($message);
		$curl = curl_init();
		curl_setopt_array($curl, array(
    ...
    function deny($vars, &$errors) {
		...
		$staff=Staff::lookup($_SESSION['_staff']['ID']);
		$t_type = 'T';
		$t_title = $staff->getName().' deny the query';
		$t_body = $vars['note'];
		$staffId = $_SESSION['_staff']['ID'];
		$poster = $staff->getName();
		$default_stauts ='7';
	...
	function set_category($vars, &$errors) {
		...
		$staff=Staff::lookup($_SESSION['_staff']['ID']);
		$t_type = 'C';
		$t_title = $staff->getName().' added the query category';
		$t_body = 'Query is updated by '.$staff->getName();
		$staffId = $_SESSION['_staff']['ID'];
		$poster = $staff->getName();
		$default_stauts = $query->complaint_status_title();
		$source = '';
        $sql_thread=' INSERT INTO '.TICKET_THREAD_TABLE.' SET created=NOW() '
            .' ,thread_type='.db_input($t_type)
            .' ,ticket_id='.db_input($id)
	...
	function checkOverdue() {
        ...
        if(($res=db_query($sql)) && db_num_rows($res)) {
            while(list($id)=db_fetch_row($res)) {
                if(($query=Query::lookup($id)) && $query->markOverdue())
                    $query->logActivity('Query Marked Overdue', 'Query flagged as overdue by the system.');
            }
        } else {
