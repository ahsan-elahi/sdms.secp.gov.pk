...
elseif(!@$thisstaff->isStaff() || !$ticket->checkStaffAccess($thisstaff)) 
die('Access Denied');
...
function get_subcategory(cat_id,s_id){
	...
	data: "cat_id="+cat_id+"&next_tiers="+next_tiers+"&isnature=0",
...
function get_focal_person(dept_id){
    ...
    if(msg.trim()==0)
    {
        $('#deptId').get(0).selectedIndex = 0;
        alert('No Focal Person is assign to Selected Department. Contact Administrator!!!');
...
<div class="row-fluid">
    ...
    <div class="info"> 
    <?php 
	$sql_get_first_dept = "Select * from sdms_ticket_event where ticket_id ='".$ticket->getId()."' order by timestamp ASC LIMIT 1";
    $res_get_first_dept=mysql_query($sql_get_first_dept);
    $row_get_first_dept=mysql_fetch_array($res_get_first_dept); 
	?>                                                               
    <ul class="rows">
    <li class="heading">Department Info <?php if($row_get_first_dept['dept_id']!=$ticket->getDeptId()){ ?> 
      <button class="btn btn-info" style="float:right;" id="popup_action_old_dept" style="padding: 0 10px;"    type="button">Department Info By User</button>
    <?php  } ?></li>
    <li>
    ...
    <li>
    <div class="title">Registration Office*</div> 	 	 	 	 	
    <div class="text"><?php 
    ...
    <?php  
	if($row_get_first_dept['dept_id']!=$ticket->getDeptId()){
	?>
     
     
 
      
    <div class="dialog" id="b_popup_action_old_dept" style="display: none;" title="Department Info By User">
    ...
    <li>
    <div class="title">Department:</div>
    <div class="text"><?php echo Format::htmlchars($dept->getNameById($row_get_first_dept['dept_id'])); ?></div>
    </li>
    <?php 
    	 if($row_get_first_dept['dept_id']==2){
    $sql_securities_markets="Select * from sdms_ticket_capital_markets where complaint_id ='".$ticket->getId()."'";
    $res_securities_markets=mysql_query($sql_securities_markets);
    $row_securities_markets=mysql_fetch_array($res_securities_markets);
    ?>
    ...
    <?php }
    else if($row_get_first_dept['dept_id']==3){
    $sql_insurance="Select * from sdms_ticket_insurance where complaint_id ='".$ticket->getId()."'";
    $res_insurance=mysql_query($sql_insurance);
    $row_insurance=mysql_fetch_array($res_insurance);
    ?>				
    ...
    <?php }
    else if($row_get_first_dept['dept_id']==4){
    $sql_ticket_scd="Select * from sdms_ticket_scd where complaint_id ='".$ticket->getId()."'";
    $res_ticket_scd=mysql_query($sql_ticket_scd);
    $row_ticket_scd=mysql_fetch_array($res_ticket_scd);
    ?>
    ...
    <?php }
    else if($row_get_first_dept['dept_id']==5){
    $sql_ticket_e_services="Select * from sdms_ticket_e_services where complaint_id ='".$ticket->getId()."'";
    $res_ticket_e_services=mysql_query($sql_ticket_e_services);
    $row_ticket_e_services=mysql_fetch_array($res_ticket_e_services);
    ?>
    ...
    <?php }
    else if($row_get_first_dept['dept_id']==6){
    $sql_ticket_cr="Select * from sdms_ticket_cr where complaint_id ='".$ticket->getId()."'";
    $res_ticket_cr=mysql_query($sql_ticket_cr);
    $row_ticket_cr=mysql_fetch_array($res_ticket_cr);
    ?>
    ...
    <?php }
    else if($row_get_first_dept['dept_id']==18){
    $sql_ticket_cs="Select * from sdms_ticket_cs where complaint_id ='".$ticket->getId()."'";
    $res_ticket_cs=mysql_query($sql_ticket_cs);
    $row_ticket_cs=mysql_fetch_array($res_ticket_cs);
    ?>
    ...
    <?php if($row_attachments['evidence_7']!='' &&  (file_exists('upload/'.$row_attachments['evidence_7'])) ){ ?>
    <li>
    <div class="title">Verification:</div>
    <div class="text"><a href="attachment_download.php?file=<?php echo $row_attachments['evidence_7']; ?>"><img src="img/download-btn.png" ></a></div>
    </li>
    <?php }?>
...
<div class="span12">
        <div class="block-fluid ucard">
                        <div class="info">                                                                
                            <ul class="rows">
                            <li class="heading">Complaint Details
                            <?php 
...
<?php echo $errors['transfer']; ?>
            ...
            <div class="span9">  Ticket is currently in <b><?php echo $ticket->getDeptName(); ?></b> department with Admin
            <?php echo $errors['transfer']; ?>
...
<?php echo $errors['transfer']; ?>
            ...
            ?>
             ...
             <br><input type="checkbox" name="ishandel_reassign" id="" value="1" />Re-Assigned Handel Complaint
            <input type="hidden" name="assignId" id="assignId_transfer" value="" />
            <input type="hidden" name="complaint_status" value="7" >
            </div>
            </div>
...
<div class="span9">
<select onChange="get_subcategory(this.value,this.id);" id="select_1" required>
...
<div style="display:none;" class="dialog" id="ba_btn_thread" title="Complaint # <?php echo $ticket->getExtId(); ?> Description">
</div>
