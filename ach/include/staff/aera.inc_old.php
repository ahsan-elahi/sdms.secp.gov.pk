...
<div class="page-header"><h1>Complaints  <small>Area Report</small></h1></div>
<div class="row-fluid">
...
<input type="hidden" name="status" value="<?php echo $statusss?>" >
    <div class="row-fluid">
        ...
        if($result && ($num=db_num_rows($result))):
            ...
            $today_date='NOW()';
            $sql_this_week="SELECT count(ticket_id) as this_week FROM sdms_ticket WHERE created>='".$start_thisweekdate."' AND created<='".$today_date."' AND location ='".$row['location']."'";
            ...
            $sql_last_week="SELECT count(ticket_id) as last_week FROM sdms_ticket WHERE created>='".$start_lastweekdate."' AND created<='".$start_thisweekdate."' AND location ='".$row['location']."'";
            ...
            $sql_this_month="SELECT count(ticket_id) as this_month FROM sdms_ticket WHERE created>='".$start_thismonthdate."' AND created<='".$today_date."' AND location ='".$row['location']."'";
            ...
            $sql_last_month="SELECT count(ticket_id) as last_month FROM sdms_ticket WHERE created>='".$start_lastmonthdate."' AND created<='".$start_thismonthdate."' AND location ='".$row['location']."'";
            ...
            $sql_this_year="SELECT count(ticket_id) as this_year FROM sdms_ticket WHERE created>='".$start_thismonthdate."' AND created<='Now()' AND location ='".$row['location']."'";
