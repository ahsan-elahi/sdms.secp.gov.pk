...
{
	$dept_add .= ' AND dept_id = '.$thisstaff->getDeptId().' OR dept_p_id = '.$thisstaff->getDeptId().'';
...
$query = "SELECT * FROM `table_name` WHERE DATE(created) >= '".date('Y-m-d',strtotime($_POST['from_date']))."' AND DATE(created) <= '".date('Y-m-d',strtotime($_POST['to_date']))."'";
...
<div class="row-fluid">
  <div class="span3" style="float:right;">
    <p align="right" style="float:right;"> <a href="departmental_summary_report.csv">
      ...
      <form action="" method="post" id="save" enctype="multipart/form-data">
        ...
        <table cellpadding="0" cellspacing="0" width="100%" class="table"  >
          <tr>
            ...
            <td><input type="text" name="from_date" id="Datepicker" required value="<?php echo $_POST['from_date']; ?>" ></td>
            <th width="20%" style="padding-top:12px;">To Date</th>
            <td><input type="text" name="to_date" id="Datepicker1" required value="<?php echo $_POST['to_date']; ?>" ></td>
          </tr>
          <tr>
...
$num_dept_comp = 0;
$sql_dept_comp = "SELECT * FROM `sdms_ticket` WHERE isquery = '0' AND dept_id='".$row_dept['dept_id']."' ";
if(!$thisstaff->isAdmin())
{  
  $sql_dept_comp.= "AND ( staff_id ='".$thisstaff->getId()."' OR ticket_id IN (SELECT DISTINCT(`ticket_id`) as ticket_id FROM sdms_ticket_event WHERE staff_id='".$thisstaff->getId()."' AND `state` = 'assigned') )";
}

$sql_dept_comp .= "  ".$from_to_date."";
...
$sql_status_comp = "SELECT * FROM `sdms_ticket` WHERE isquery = '0' ";
if(!$thisstaff->isAdmin())
{  
$sql_status_comp .="AND ( staff_id ='".$thisstaff->getId()."' OR ticket_id IN (SELECT DISTINCT(`ticket_id`) as ticket_id FROM sdms_ticket_event WHERE staff_id='".$thisstaff->getId()."' AND `state` = 'assigned') )";  
}
$sql_status_comp .=" AND complaint_status IN (SELECT status_id FROM `sdms_status` WHERE p_id='".$row_status['status_id']."') AND  dept_id='".$row_dept['dept_id']."' ".$from_to_date."";
...
?>
            ...
            <?php
                ...
                $sql_status_comp_inner = "SELECT * FROM `sdms_ticket` WHERE isquery = '0'";
                if(!$thisstaff->isAdmin())
                {  
                $sql_status_comp_inner .=" AND ( staff_id ='".$thisstaff->getId()."' OR ticket_id IN (SELECT DISTINCT(`ticket_id`) as ticket_id FROM sdms_ticket_event WHERE staff_id='".$thisstaff->getId()."' AND `state` = 'assigned') )";
                }
                $sql_status_comp_inner .="   AND complaint_status='".$row_sub_status_inner['status_id']."' AND  dept_id='".$row_dept['dept_id']."' ".$from_to_date."";
...
$sub_sub_status .= "]},";
          ...
          <tr id="total">
            ...
            <td><span class="Icon <?php echo $icon;?>" align="right"><b><span align="right">
			<a href="tickets.php?a=search<?php echo $date_range; ?>">
			<?php echo $subnum_dept_comp; ?>
            </a> </span></b><br />
              </span></td>
...
while($row_status=mysql_fetch_array($res_status)){
	?>
    <td><b>
	 <a href="tickets.php?a=search&primary_stutus=<?php echo $row_status['status_id'].$date_range; ?>">
	<?php echo $sub_status_count[$nos]; $csv .= '"' . $sub_status_count[$nos] . '",'; ?></b>
    </a></td>
    <?php
$sql_sub_status="SELECT * FROM `sdms_status` WHERE p_id='".$row_status['status_id']."'";
$res_sub_status=mysql_query($sql_sub_status);
...
$sql_sub_status_count="SELECT * FROM `sdms_ticket` WHERE complaint_status = '".$row_sub_status['status_id']."' AND isquery = '0'";
$res_sub_status_count=mysql_query($sql_sub_status_count);
$num_sub_total =  mysql_num_rows($res_sub_status_count);

$href_sub_total = "tickets.php?a=search&cstatus=".$row_sub_status['status_id'].$date_range;
	  ?>
            <th style="display:none;" class="status_<?php echo $row_status['status_id']; ?>">
            <a href="<?php echo $href_sub_total; ?>"><?php echo $num_sub_total ?></a></th>
            <?php }  ?>
            <?php $nos++; } ?>
            <?php // } 
		   $csv.="\n";
...
{
$sql_dept="SELECT * FROM `sdms_department` WHERE 1 AND dept_p_id = '".$thisstaff->getDeptId()."'";
$res_dept=mysql_query($sql_dept);
$num_dept = mysql_num_rows($res_dept);	
if($num_dept>0){
 ?>
  ...
  <div class="span12">
    ...
    <div class="block-fluid table-sorting clearfix">
      <table cellpadding="0" cellspacing="0" width="100%" class="table">
        <thead>
          <tr>
            <th rowspan="2">Sub Department of Complaint</th>
            <th rowspan="2" align="center">Total Complaints</th>
            <th colspan="5" align="center">Status</th>
          </tr>
          <tr>
...
?>
          <tr>
            <th><span style="float:left; width:350px;"  ><?php echo $row_dept['dept_name']; $csv .= '"' . $row_dept['dept_name'] . '",';?></span></th>
            <td><b><span align="right"><a href="tickets.php?a=search&deptId=<?php echo $row_dept['dept_id'].$date_range; ?>">
              <?php 
	echo $num_dept_comp;
...
$num_status_comp += mysql_num_rows($res_status_comp);
            ...
            <th><a href="tickets.php?a=search&deptId=<?php echo $row_dept['dept_id']; ?>&primary_stutus=<?php echo $row_status['status_id'].$date_range; ?>"><?php echo $num_status_comp; $sub_status_count[$i] +=$num_status_comp; $csv .= '"' . $num_status_comp . '",'; ?></a>
              </td>
              <?php
