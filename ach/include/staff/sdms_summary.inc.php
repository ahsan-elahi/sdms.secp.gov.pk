...
<div class="row-fluid">
   <div class="span12">                    
        ...
        <div class="block-fluid table-sorting clearfix">
          <form action="sdms_summary.php" method="GET">
          ...
          <tr>
            ...
            <?php for ($i = date('m'); $i >= 1; $i--) { ?>
            <option value="<?php echo $i; ?>"  <?php if ($_GET['fdate']  == $i) { ?> selected="selected" <?php } ?> >
...
<!--<div class="span3" style="float:right;">
    ...
    <div class="row-fluid">
        <div class="span12">                    
			...
			while($m_row=mysql_fetch_array($m_res))
			{
				...
				{
				if ($_GET['fdate']) {
					$_SESSION['date']=$_GET['fdate'];
				$qselect='SELECT count(ticket_id) as one_total FROM '.TICKET_TABLE.'  WHERE topic_id="'.$sub_row['topic_id'].'" AND MONTH(created)="'.$_GET['fdate'].'"';
				}
				else{
			...
			<?php 
			if ($_GET['fdate']) {
				$sql_gender='SELECT count(ticket_id) as gender_total ,gender FROM sdms_ticket Where MONTH(created)="'.$_GET['fdate'].'" group by gender ';
				}
				else{
            ...
            <?php 
				if ($_GET['fdate']) {
				$sql_location='SELECT count(ticket_id) as location_total ,applicant_location FROM sdms_ticket Where MONTH(created)="'.$_GET['fdate'].'" group by applicant_location ';
				}
				else
				{
    ...
    <div class="row-fluid">
        <div class="span12">                    
			...
			if($_GET['Search'] && $_GET['fdate']!='')
            {
			$sql_this_month="SELECT count(ticket_id) as this_month FROM sdms_ticket WHERE Month(created)='".$_GET['fdate']."'";
			$res_this_month=mysql_query($sql_this_month);
            $row_this_month=mysql_fetch_array($res_this_month);	
			echo '<td>'.date('F', mktime(0, 0, 0, $_GET['fdate'], 10)).'</td>';
			}
			else
			{
			$today_date='NOW()';
			...
			if($_GET['Search'] && $_GET['fdate']!='')
            {
			$sql_follow_up="SELECT count(ticket_id) as total_followup FROM sdms_ticket WHERE status='open' AND Month(created)='".$_GET['fdate']."'";
			...
			if($_GET['Search'] && $_GET['fdate']!='')
            {
			$sql_close="SELECT count(ticket_id) as total_closed FROM sdms_ticket WHERE status='closed' AND Month(created)='".$_GET['fdate']."'";
