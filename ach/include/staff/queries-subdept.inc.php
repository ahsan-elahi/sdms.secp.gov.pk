...
if($_REQUEST['status']) { //Query string status has nothing to do with the real status used below; gets overloaded.
    $qstr.='ticket.status='.urlencode($_REQUEST['status']);
}
...
$qwhere =' WHERE ( ';//        .'  ticket.staff_id ='.db_input($thisstaff->getId());
    ...
    $qwhere.='  ticket.dept_id IN ('.($depts?implode(',', db_input($depts)):0).')';
	//}else{
	//$qwhere .=" OR ticket.ticket_id IN (SELECT DISTINCT(`ticket_id`) as ticket_id FROM ".TICKET_EVENT_TABLE." WHERE staff_id='".db_input($thisstaff->getId())." 
	  //     ' AND `state` = 'assigned') ";	
	//	}
/*
...
//    $qwhere.=' AND ticket.staff_id != '.$thisstaff->getId().' AND ticket.status='.db_input(strtolower($status));    
   // $qwhere.=' AND ticket.staff_id != '.$thisstaff->getId().' ';    
...
<div class="page-header"><h1>Queries <small>And Details</small></h1></div>  
<div class="row-fluid">
<div class="span4" style="float:left">
<form action="queries.php" method="get">
    <?php  //csrf_token(); ?>
    <input type="hidden" name="a" value="search">
     <input type="hidden" name="action" value="subdept">
...
<?php /*?><div class="span3" style="float:right">
                        ...
                        <table cellpadding="0" cellspacing="0" width="100%" class="table" >
                            <thead>
                                <tr>
                                    ...
                                    <th width="14%">Sub Status</th>
                                    <!--<th width="14%">Location</th>
                                    <th width="19%">Lying With</th> 
...
if($row['status']=='open' && $total_days > '15'){
$alret='style="background-color: #fee7e7 !important;"';
}?>
       ...
       <td <?php echo $alret; ?> title="<?php echo $row['email']; ?>" nowrap>
              ...
              <a class="Icon <?php echo strtolower($row['source']); ?>Ticket ticketPreview" title="Preview Query " 
				 href="queries.php?action=subdeptview&subdeptviewid=<?php echo $row['ticket_id']; ?>"><?php echo $tid; ?></a>

                </td>     

          
          <td nowrap <?php echo $alret; ?>>
		  <?php echo Format::db_date($row['created']); ?></td>
          <td <?php echo $alret; ?>>
          <?php echo $subject; ?></td>                     
          <td nowrap <?php echo $alret; ?>>
          &nbsp;<?php echo Format::truncate($row['name'],22,strpos($row['name'],'@')); ?>&nbsp;</td>
          <td><?php echo $row['staff']; ?></td> 
         <?php /*?> <td nowrap <?php echo $alret; ?>><?php echo $row['District']; ?></td><?php */?>
          <?php 
		 ...
		 $sql_fetchstatus = "Select * from sdms_status where status_id='".$row['complaint_status']."'";
		  ...
		  ?>
          <td><?php echo $row_fetchstatus['status_title']; ?></td>
          <td><?php echo $row_fetchpstatus['status_title']; ?></td>   
          <?php  
...
<!--                <input class="btn btn-inverse" type="submit" name="close" value="Close" id="close_a" >
    ...
    <div class="block">                                                               
            <ul class="rows">
                ...
                <li>
                    <div class="title">Applicant Cell :</div>
    ...
    &nbsp;<font class="error">&nbsp;<?php echo $errors['district']; ?></font>
                ...
                <li>
                    <div class="title">Type Of Query:</div>
