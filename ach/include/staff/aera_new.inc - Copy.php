...
myWindow=window.open("aera_new_print.php","Print Report","toolbar=yes,width=800px,height=14031px");
myWindow.print() ;
//myWindow.close();
}
</script>
...
?>
<?php 
$qselect='SELECT count(ticket_id) as ticketno,location';
$qfrom='FROM '.TICKET_TABLE.' t';
$qwhere='group by location';
...
?>
<div class="page-header"><h1>Complaints  Volume Summary<small> By Area</small></h1></div>
<div class="row-fluid">
...
<div class="row-fluid">
   <div class="span12">                    
        <div class="head clearfix">
            ...
            <h1><?php echo 'Complaint Volume Summary: By Area'; ?></h1>                               
        </div>
    ...
    <tbody class="" page="1">
        ...
        <tr>
            ...
            <span style="float: left; width:350px;"><?php if($row['location']!="") echo $row['location']; else echo "No Location";?></span></a></th>
            &nbsp;&nbsp;
            <td>
            <?php 
			$check="";
            ...
            <td>
            <?php $sql_daily="SELECT count(ticket_id) as daily FROM ost_ticket WHERE 0 >=TIMESTAMPDIFF(DAY,created,NOW()) AND location ='".$row['location']."'";
            ...
            $today_date='NOW()';
            $sql_this_week="SELECT count(ticket_id) as this_week FROM ost_ticket WHERE created>='".$start_thisweekdate."' AND created<='".$today_date."' AND location ='".$row['location']."'";
            ...
            $sql_last_week="SELECT count(ticket_id) as last_week FROM ost_ticket WHERE created>='".$start_lastweekdate."' AND created<='".$start_thisweekdate."' AND location ='".$row['location']."'";
            ...
            $sql_this_month="SELECT count(ticket_id) as this_month FROM ost_ticket WHERE created>='".$start_thismonthdate."' AND created<='".$today_date."' AND location ='".$row['location']."'";
            ...
            $sql_last_month="SELECT count(ticket_id) as last_month FROM ost_ticket WHERE created>='".$start_lastmonthdate."' AND created<='".$start_thismonthdate."' AND location ='".$row['location']."'";
            ...
            $sql_this_year="SELECT count(ticket_id) as this_year FROM ost_ticket WHERE created>='".$start_thismonthdate."' AND created<='Now()' AND location ='".$row['location']."'";
        ...
        <th><span style="float: left; width:350px;">Total</span></th>
                &nbsp;&nbsp;
            <td><b><?php echo $all_complaints; ?></b></td>
