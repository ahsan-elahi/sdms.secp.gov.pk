...
$staffId=0; //Nothing for now...TODO: Allow admin and manager to limit Queries to single staff level.
...
/* 
   ...
   User can also see Queries assigned to them regardless of the Query's dept.
...
if($_REQUEST['action']=='escalation')
{
$t_esc_days = explode("-",$_REQUEST['esc_days']);
$ecs_nos=1;
foreach($t_esc_days as $p_esc_day)
{
	if($ecs_nos==1){
$notify_days .= " AND (notify_days = '".$p_esc_day."' ";
	}else{
$notify_days .= " OR notify_days = '".$p_esc_day."'";	
	}
	$ecs_nos++;
...
$notify_days .= ')';
$qwhere .=" AND ticket.ticket_id IN (SELECT DISTINCT(`ticket_id`) as ticket_id FROM `sdms_notify` WHERE staff_id = '".$_REQUEST['esc_staffid']."' ".$notify_days.") ";
}
...
}elseif($status=='deny')
{
	...
	//echo 'wellcome';exit;
	    ...
	    $qwhere.=' AND ticket.isrejected=1 AND ticket.status = "open"';    
...
if($staffId && ($staffId==$thisstaff->getId())) { //My Queries

    $results_type='Assigned Queries';
...
}elseif(!strcasecmp($status, 'open') && !$search) { //Open queue (on search OPEN means all open Queries - regardless of state).

    //Showing answered Queries on open queue??
...
if($search):
    ...
    if($_REQUEST['respondent']) {
        ...
        $qwhere.=' AND ticket.comp_against='.db_input($_REQUEST['respondent']);
...
//ahsan code to match with closed date which close status select
if($_REQUEST['status'] =='closed' || $_REQUEST['primary_stutus'] =='2')
$db_dated = 'closed';
else
$db_dated = 'created';

    //dates
//echo $_REQUEST['startDate'].'<br>'.$_REQUEST['endDate'].'<bR>';
...
//echo $startTime.'<br>'.$endTime;exit;
    if( ($startTime && $startTime>time()) or ($startTime>$endTime && $endTime>0)){
        ...
        if($startTime){
            ...
            $qwhere.=' AND DATE(ticket.created)>=DATE(FROM_UNIXTIME('.$startTime.'))';
        ...
        if($endTime){
            ...
            $qwhere.=' AND DATE(ticket.created)<=DATE(FROM_UNIXTIME('.$endTime.'))';
...
$qselect ='SELECT DISTINCT ticket.ticket_id,lock_id,ticket.complaint_status,ticket_voice,ticketID,ticket.dept_id,dist.*,ticket.district,ticket.staff_id,ticket.closed,ticket.team_id '
...
$pageNav->setURL('queries.php',$qstr.'&sort='.urlencode($_REQUEST['sort']).'&order='.urlencode($_REQUEST['order']));
...
$_SESSION['queries_list_session'] = $seccion_query;
...
{
	echo $query.'<br>';
	echo $_SESSION['queries_list_session'];
}
$hash = md5($query);
...
if(!$results_type)
    ...
    $results_type = ucfirst($status).' Queries';
...
<div class="page-header"><h1>Queries <small></small></h1></div>  
...
<div class="row-fluid">
  <div class="span6" style="float:right;">
    <p align="right" style="float:right;padding-left:20px;"><span class="box_new assigned_new">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;Assigned Complaint</p>
    <p align="right" style="float:right;padding-left:20px;"><span class="box_new handled_new">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;Handled Complaint</p>
    <p align="right" style="float:right;padding-left:20px;"><span class="box_new pending_new">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;Pending Complaint</p>
  </div>
</div>
    <form action="queries.php" method="POST" name='tickets' id="status-form">
    <?php csrf_token(); ?>
                        ...
                        <table cellpadding="0" cellspacing="0" width="100%" class="table" >
                            ...
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <?php $csv.='"Subject"'.','; ?>
                                    <th width="14%">Complainant</th> 
                                    <?php $csv.='"Submitted By"'.','; ?>
                                    <!--<th width="14%">Priority</th>-->
                                    <th width="14%">Handling Officer</th>
                                    <?php $csv.='"Assigned to"'.','; ?>
                                    ...
                                    if($thisstaff->isAdmin())
                                    {?>	
                                    <th width="14%">Nature of Complaint</th>
                                    <?php $csv.='"Voice"'.','; ?>
                                    <?php }
                                    else{?>
...
?>
        ...
        <tr id="<?php echo $row['ticket_id']; ?>">
          ...
          <td <?php echo $alret; ?> title="<?php echo $row['email']; ?>" nowrap>
              ...
              <a class="Icon <?php echo strtolower($row['source']); ?>Ticket ticketPreview" title="Preview Query " 

                href="queries.php?id=<?php echo $row['ticket_id']; ?>"><?php echo $tid; ?></a>
 <?php $csv .= '"' . $row['ticket_id'] . '",'; ?>
                </td>     
          ...
          <?php if($row['reopened']!=''){ ?>
              <br><span class="label label-important">Reopen: <?php echo Format::db_date($row['reopened']); ?></span>
          ...
          <a <?php if($flag) { ?> class="Icon <?php echo $flag; ?>Ticket" title="<?php echo ucfirst($flag); ?> Query " <?php } ?> 

                href="queries.php?id=<?php echo $row['ticket_id']; ?>"><?php echo $subject; ?>
                <?php $csv .= '"' .$subject. '",'; ?>
                </a>
...
<td><?php echo $row['staff'];  $csv .= '"'.$row['staff'].'",'; ?></td> 
<!--<td><?php //echo $row['District'];  $csv .= '"'.$row['District'].'",'; ?></td> -->
      <?php /*?><td style="background-color:<?php echo $row['priority_color']; ?>;">
...
<td  <?php echo $alret; ?>><?php 
  ...
  <?php  
	if($thisstaff->isAdmin())
	{?>	
	    <td nowrap <?php echo $alret; ?>><?php echo $row['helptopic']; ?></td>
<?php }else{ 	
	?>
...
<?php }?>   
 ...
 $csv.="\n";
        ...
        $ferror='There are no Query s here. (Leave a little early today).';  
...
<!--                <input class="btn btn-inverse" type="submit" name="close" value="Close" id="close_a" >
    ...
    <div style="display:none;" class="dialog_show" id="ba_btn_close_a" title="Please Confirm" >
        ...
        <p >
            ...
            Are you sure want to <b>close</b> selected open Queries?

        </p>

        <p class="confirm-action" style="display:none;" id="reopen-confirm">

            Are you sure want to <b>reopen</b> selected closed Queries?
    ...
    <!--<div style="display:none;" class="dialog_show" id="confirm-action">
        ...
        <p class="confirm-action" style="display:none;" id="mark_overdue-confirm">
            ...
            Are you sure want to flag the selected Queries as <font color="red"><b>overdue</b></font>?

        </p>

        <p class="confirm-action" style="display:none;" id="delete-confirm">

            <font color="red"><strong>Are you sure you want to DELETE selected Queries?</strong></font>

            <br><br>Deleted Queries CANNOT be recovered, including any associated attachments.
    ...
    <div class="dialog" id="b_popup_4" style="display: none;" title="Advanced Query Search">

    <form action="queries.php" method="get" id="search" name="search">
    ...
    <div class="block">                                                               
            ...
            <ul class="rows">
                ...
                <li>
                    ...
                    <select name="primary_stutus"  id="status" style="width:300px;">
    ...
    &nbsp;<font class="error">&nbsp;<?php echo $errors['district']; ?></font>
                ...
                <li>
                    ...
                    <div class="text"><select name="m_topicId" onChange="get_Sub_Com(this.value);">

                      <option value="" selected="selected">--Select Type--</option>

                        <?php

    if($topics=Topic::getPublicHelpTopics($thisstaff->getDeptId())) {
