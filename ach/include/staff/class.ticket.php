...
class Ticket {
    ...
    function onAssign($assignee, $comments, $alert=true) {
        ...
        $this->logNote('Complaint Assigned to '.$assignee->getName(), $comments, $assigner, false);
...
*/
   ...
   /*============== Static functions. Use Ticket::function(params); ==================*/
    ...
    function create($vars, &$errors, $origin, $autorespond=true, $alertstaff=true) {
		...
		if($vars['email']=="" & $vars['nic']=="")
		$vars['email'] = "noreply@sdms.phc.pk";
		else if($vars['email']=="")
		$vars['email'] = $vars['nic']."@git.gov.pk";
		/************************Minde Code*****************************/	
        ...
        //We are ready son...hold on to the rails.
          ...
          $sql='INSERT INTO '.TICKET_TABLE.' SET created=NOW() '
			...
			.' ,comp_against='.db_input($vars['comp_against'])
			.' ,against_dept='.db_input($vars['comp_against_dept'])
			.' ,address='.db_input($vars['address'])
			.' ,ip_address='.db_input($ipaddress)
			.' ,nic='.db_input($vars['nic'])
			.' ,gender='.db_input($vars['gender'])
			.' ,applicant_address='.db_input($vars['user_address'])
...
$uname = 'carl@telewin.co.uk';
		...
		$message = 'Your Complaint has been successfully registered. UCN:'.$extId.' PINCODE:'.$pincode.'';
...
$uname = 'carl@telewin.co.uk';
		...
		$from = 'HRD PHC';
		$selectednums = '+92'.substr($vars['phone'],1);
		$message = 'Your Pincode has been successfully Change. UCN:'.$extId.' PINCODE:'.$pincode.'';
		$message = urlencode($message);
		 */
		// Prepare data for POST request
		//$data = "uname=".$uname."&pword=".$pword."&message=".$message
    ...
    function open($vars, &$errors) {
        ...
        if($vars['source'] && !in_array(strtolower($vars['source']),array('email','phone','physical','fax')))
