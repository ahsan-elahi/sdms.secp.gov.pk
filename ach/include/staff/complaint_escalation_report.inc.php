...
<div class="page-header">   
  <h1>Complaints Escalation <small></small></h1>
</div>
<div class="row-fluid">
<div class="span12">                    
...
$sql_escalations ="select distinct notify_days, count(notify_days) as CountOf from sdms_notify where staff_id = '".$row_poclist['staff_id']."' AND ticket_id IN(SELECT DISTINCT(`ticket_id`) as ticket_id FROM sdms_ticket where isquery =0) ".$from_to_date." group by notify_days";
  ...
  <tr>
	...
	?></td>
<td><a href="tickets.php?esc_staffid=<?php echo $row_poclist['staff_id'].$date_range; ?>&esc_days=3&action=escalation"><?php echo $days3; ?></a></td>
<td><a href="tickets.php?esc_staffid=<?php echo $row_poclist['staff_id'].$date_range; ?>&esc_days=7&action=escalation"><?php echo $days7; ?></a></td>
<td><a href="tickets.php?esc_staffid=<?php echo $row_poclist['staff_id'].$date_range; ?>&esc_days=15&action=escalation"><?php echo $days15; ?></a></td>
<td><a href="tickets.php?esc_staffid=<?php echo $row_poclist['staff_id'].$date_range; ?>&esc_days=30&action=escalation"><?php echo $days30; ?></a></td>
<td><a href="tickets.php?esc_staffid=<?php echo $row_poclist['staff_id'].$date_range; ?>&esc_days=45&action=escalation"><?php echo $days45; ?></a></td>
<td><a href="tickets.php?esc_staffid=<?php echo $row_poclist['staff_id'].$date_range; ?>&esc_days=3&action=escalation"><?php echo $days3; ?></a></td>
<td><a href="tickets.php?esc_staffid=<?php echo $row_poclist['staff_id'].$date_range; ?>&esc_days=7-15&action=escalation"><?php echo $days7+$days15; ?></a></td>
<td><a href="tickets.php?esc_staffid=<?php echo $row_poclist['staff_id'].$date_range; ?>&esc_days=30&action=escalation"><?php echo $days30; ?></a></td>
<td><a href="tickets.php?esc_staffid=<?php echo $row_poclist['staff_id'].$date_range; ?>&esc_days=45&action=escalation"><?php echo $days45; ?></a></td>
  </tr>
  <?php } ?>
...
<?php if($_POST['dept_id']!=''){ ?>

<?php
$sql_deptstafflist="Select * from sdms_staff where 1 ".$dept_add." AND (group_id='5' OR group_id='8') ";
$res_deptstafflist = mysql_query($sql_deptstafflist);
while($row_deptstafflist = mysql_fetch_array($res_deptstafflist)){
	  $days3 = 0;
	  $days7 = 0;
	  $days15 = 0;
	  $days30  = 0;
	  $days45  = 0;
	$staff_list  .= "'".$row_deptstafflist['firstname'].' '.$row_deptstafflist['lastname'].'<br>('.$row_deptstafflist['username'].')'."',";  
$sql_dept="SELECT * FROM `sdms_department` WHERE dept_id='".$row_poclist['dept_id']."'";
$res_dept=mysql_query($sql_dept);
$row_dept = mysql_fetch_array($res_dept);
$dept_name = $row_dept['dept_name'];

$sql_escalations ="select distinct notify_days, count(notify_days) as CountOf from sdms_notify where staff_id = '".$row_deptstafflist['staff_id']."' AND ticket_id IN(SELECT DISTINCT(`ticket_id`) as ticket_id FROM sdms_ticket where isquery =0) ".$from_to_date." group by notify_days";
//echo $sql_escalations.'<br>';
	 $res_escalations = mysql_query($sql_escalations);
     while($row_escalations = mysql_fetch_array($res_escalations)){
	 ...
	 { 
	 $days3 = $row_escalations['CountOf'];
	 $days3_data .= $row_escalations['CountOf'].',';
	  }
	 ...
	 { 
	 $days15 = $row_escalations['CountOf'];
	 $days15_data .= $row_escalations['CountOf'].',';
	  }
	 
	 if($row_escalations['notify_days'] == 30 )
	 { $days30 = $row_escalations['CountOf'];
	 $days30_data .= $row_escalations['CountOf'].',';
	  }
	 
	 if($row_escalations['notify_days'] == 45 )
	 { $days45 = $row_escalations['CountOf'];
	 $days45_data .= $row_escalations['CountOf'].',';
	  }
...
echo $closed . '<br>';
echo $sub_judice . '<br>';
echo $referred_to_third_party . '<br>';
echo $resolved. '<br>';
exit;
$data = "{
        name: '3 Days',
...
$data = "{
	...
	//echo $data;
//$data = "{ name: 'In Process', data: [0,0,4,0,0] }, { name: 'Closed', data: [8,9,1,0,0] }, { name: 'Sub Judice', data: [0,0,0,0,0] }, { name: 'Referred to third party', data: [0,0,0,0,0] }, { name: 'Resolved', data: [1,2,0,0,0] }";
 ?>
<div class="row-fluid">
            <div class="span12">
            <div class="head clearfix">
...
Highcharts.chart('container', {
    ...
    title: {
        text: '<?php echo $dept_name; ?> chart'
    },
    xAxis: {
        categories: [<?php echo $staff_list; ?>]
    },
    yAxis: {
        min: 0,
        title: {
            text: 'Total Escalation per days'
        },
        stackLabels: {
            enabled: true,
            style: {
