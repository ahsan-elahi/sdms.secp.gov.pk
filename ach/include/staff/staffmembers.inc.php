...
$seccion_query = "$select $from $where GROUP BY staff.staff_id ORDER BY $order_by";
$_SESSION['staff_list_session'] = $seccion_query;
...
<tr>
<th width="20%" style="padding-top:12px;">By Group</th>
<td  >
<select name="gid" >
<option value="">--Select Group--</option>
<?php 
$sql_get_groups='SELECT * from  sdms_groups WHERE 1 ';
$res_get_groups = mysql_query($sql_get_groups);
while($row_group = mysql_fetch_array($res_get_groups)){
?>
<option value="<?php echo $row_group['group_id'] ;?>" <?php if($row_group['group_id']==$_REQUEST['gid']){ ?> selected <?php }?>><?php echo $row_group['group_name'] ;?></option>
<?php } ?>
</select>
</td>
</tr>    
...
<a href="export_staff.php"><button class="btn" type="button" ><i class="icon-print"></i> Export</button></a>
</p>
        ...
        if($res && db_num_rows($res)):
			...
			if(($color%2) == 0)
               ...
               <tr id="<?php echo $row['staff_id']; ?>" class="<?php echo $bg;?>">
                ...
                <td><?php echo $row['isactive']?'Active':'<b>Locked</b>'; ?>
                &nbsp;<?php echo $row['onvacation']?'<small>(<i>vacation</i>)</small>':''; ?></td>
                <td>
