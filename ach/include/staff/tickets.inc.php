...
*/
if(!$thisstaff->isAdmin() &&  $thisstaff->onChairman() == '1'  )
{
	$depts = $thisstaff->getDepts(); 
	$depts += $thisstaff->getSubDepts();
}else{
$depts=$thisstaff->getDepts();  
}
$qwhere =' WHERE ( '
...
if($_REQUEST['r_type']!='')
{
	...
	if($_REQUEST['r_type']=='180daysplus')
	{
		$qstr.='&r_type='.$_REQUEST['r_type'].'&from_date='.$_REQUEST['from_date'].'&to_date='.$_REQUEST['to_date'].'';	
		$qwhere .=" AND DATE(ticket.created) <=  '".$from_date."' AND DATE(ticket.created) > '".$to_date."'";
	}
	if($_REQUEST['r_type']=='365daysplus')
	{
		$qstr.='&r_type='.$_REQUEST['r_type'].'&from_date='.$_REQUEST['from_date'];	
		$qwhere .=" AND DATE(ticket.created) <=  '".$from_date."' AND DATE(ticket.created) > '".$to_date."'";
	}
	if($_REQUEST['r_type']=='730daysplus')
	{
		$qstr.='&r_type='.$_REQUEST['r_type'].'&from_date='.$_REQUEST['from_date'];	
		$qwhere .=" AND DATE(ticket.created) <=  '".$from_date."'  AND DATE(ticket.created) > '".$to_date."'";
	}
	if($_REQUEST['r_type']=='1095daysplus')
	{
		$qstr.='&r_type='.$_REQUEST['r_type'].'&from_date='.$_REQUEST['from_date'];	
...
{
$qwhere .= ' AND DATE(date) >= "'.date('Y-m-d',strtotime($_REQUEST['sDate'])).'" AND DATE(date) <= "'.date('Y-m-d',strtotime($_REQUEST['eDate'])).'"  ';
}
$qwhere .=" ) ";
...
if($_REQUEST['action']=='capitalmarket')
{
	$cm_type = $_REQUEST['cm_type'];
	$compay = $_REQUEST['compay'];
$sql_status="SELECT cm_broker_title as compay,count(cm_broker_title) as total ,group_concat( complaint_id ) as complaint_ids, group_concat( cm_broker_title ) AS companies FROM `sdms_ticket_capital_markets` WHERE cm_broker_title!='' AND cm_type = '".$cm_type."' AND cm_broker_title = '".$compay."'  group by `cm_broker_title` order by total desc limit 10";
...
$sql_status="SELECT `cm_broker_agent` as agent,count(`cm_broker_agent`) as total ,group_concat( complaint_id ) as complaint_ids, group_concat( `cm_broker_agent` ) AS agents FROM `sdms_ticket_capital_markets` WHERE cm_broker_agent='".$agent."' group by `cm_broker_agent` order by total desc limit 10";
$res_status=mysql_query($sql_status);
$row_status=mysql_fetch_array($res_status);
$qwhere .=" AND  ticket.ticket_id IN(".$row_status['complaint_ids'].") ";
}
...
{
	$name = $_REQUEST['name'];
$sql_status = "SELECT  count(ticket_id) as total,group_concat( ticket_id ) as complaint_ids,`email` FROM `sdms_ticket` WHERE  email!='' AND dept_id = '2' AND isquery=0 AND email NOT LIKE '%@novalidemail.pk' AND name = '".$name."'  group by `email` order by total desc limit 0,5";
$res_status = mysql_query($sql_status);
$row_status = mysql_fetch_array($res_status);
$qwhere .=" AND  ticket.ticket_id IN(".$row_status['complaint_ids'].") ";
}
...
$qwhere .=" AND ticket.source = '".$source."' ";
}

if($_REQUEST['dept_id']!='')
{
$qwhere .=" AND ticket.dept_id = '".$_REQUEST['dept_id']."'";
}


if($_REQUEST['i_type']!='')
{
$type = "AND i_type= '".$_REQUEST['i_type']."'";
$i_broker_title = "AND i_broker_title= '".$_REQUEST['i_broker_title']."'";

$qwhere .=" AND ticket.ticket_id IN (SELECT DISTINCT(`complaint_id`) as ticket_id FROM sdms_ticket_insurance WHERE 1 ".$type." ".$i_broker_title.") ";	
}


if($_REQUEST['action']=='pending_complaints')
{

	$qwhere .=' AND ticket.staff_id ='.db_input($thisstaff->getId());
$qwhere .=" AND ticket.ticket_id NOT IN (Select ticket_id from sdms_ticket_thread where  (thread_type = 'N' OR thread_type = 'R')  AND title 
NOT LIKE 'Complaint Assigned to%') ";	
	

}
if($_REQUEST['action']=='handled_complaints')
{
	$qwhere .=' AND ticket.staff_id ='.db_input($thisstaff->getId());
	$qwhere .=" AND ticket.ticket_id IN (Select ticket_id from sdms_ticket_thread where  (thread_type = 'N' OR thread_type = 'R')  AND title NOT LIKE 'Complaint Assigned to%')";
	
	//$qwhere .=" AND ticket.ticket_id NOT IN (Select ticket_id from sdms_ticket_thread where staff_id = '129' order by id desc limit 1)";	
}
if($_REQUEST['action']=='pending_handled_complaints')
{
	$qwhere .=' AND  ticket.ishandel_reassign = "1" AND ticket.staff_id ='.db_input($thisstaff->getId());	
}
if($_REQUEST['action']=='assigned_complaints')
{
	$qwhere .=' AND ticket.staff_id !='.db_input($thisstaff->getId());
}
if($_REQUEST['action']=='nearoverdue')
{
  $qwhere .=" AND TIMESTAMPDIFF(DAY,ticket.created,NOW())>=4 AND TIMESTAMPDIFF(DAY,ticket.created,NOW())<=10 AND ticket.isrejected<>'1' AND ticket.ticket_id>=4100 AND ticket.ticket_id NOT IN (select ticket_id from sdms_ticket_thread where thread_type='R') ";
  } 
//STATUS
...
if($mark_complaint) {
	...
	if(!$thisstaff->isAdmin() &&  $thisstaff->onChairman() == '1' )   
	$qwhere .=' AND  ticket.isaccepted=1 AND ticket.status = "open"';
	elseif(!$thisstaff->isAdmin() &&  $thisstaff->onChairman() == '0' )   
...
if(!$thisstaff->isAdmin() &&  $thisstaff->onChairman() == '0')
...
if($startTime == $endTime && $startTime !=0 ){
	...
	$qwhere.=' AND DATE(ticket.'.$db_dated.') >= "'.date('Y-m-d',strtotime($_REQUEST['startDate'])).'"';

            $qstr.='&startDate='.urlencode($_REQUEST['startDate']);
			
	$qwhere.=' AND DATE(ticket.'.$db_dated.') <= "'.date('Y-m-d',strtotime($_REQUEST['endDate'])).'"';
        ...
        if($startTime){
            ...
            $qwhere.=' AND ticket.'.$db_dated.'>=FROM_UNIXTIME('.$startTime.')';
        ...
        if($endTime){
            ...
            $qwhere.=' AND ticket.'.$db_dated.'<=FROM_UNIXTIME('.$endTime.')';
...
<div class="row-fluid">
  <div class="span12" style="float:right;">
  	<a href="tickets.php?action=nearoverdue"><p align="right" style="float:right;padding-left:20px;"><span class="box_new nearoverdue">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;Nearing Overdue Complaint</p></a>
    ...
    <div class="block">
      <ul class="rows">
        ...
        <li>
          ...
          <div class="text">
            <select name="primary_stutus"  id="status" style="width:300px;" onChange="get_Sub_Status(this.value)" >
