...
$sql_status="SELECT * FROM `sdms_status` WHERE p_id='0' AND status_id='1' || status_id='2'";
...
$sql_status_comp = "SELECT * FROM `sdms_ticket` WHERE isquery = '1' AND complaint_status='".$row_sub_status['status_id']."' ".$dept_add."";
...
for($i=1;$i<16;$i++)
{
$sql_1to15days = "SELECT * FROM `sdms_ticket` WHERE isquery = '1' AND complaint_status='".$row_sub_status['status_id']."' AND DATE(created) = '".$today_date."' ".$dept_add."";
...
for($i=1;$i<16;$i++)
{
$sql_16to30days = "SELECT * FROM `sdms_ticket` WHERE isquery = '1' AND complaint_status='".$row_sub_status['status_id']."' AND DATE(created) = '".$today_date."' ".$dept_add."";
...
for($i=1;$i<16;$i++)
{
$sql_31to45days = "SELECT * FROM `sdms_ticket` WHERE isquery = '1' AND complaint_status='".$row_sub_status['status_id']."' AND DATE(created) = '".$today_date."' ".$dept_add."";
...
$sql_45daysplus = "SELECT * FROM `sdms_ticket` WHERE isquery = '1' AND complaint_status='".$row_sub_status['status_id']."' AND DATE(created) <= '".$today_date."' ".$dept_add."";
...
<div class="span6">
    <div class="wBlock <?php echo $row_status['color'];?> clearfix">                        
        ...
        <div class="rSpace" style="width:55%;">
            ...
            <span>(<?php echo $t_31to45days; ?>) <b>45+ Day</b></span>
...
<!--Queries Status , Complaint Categories , Geophrical Summary, Line chart , Bar chart -->
<div class="dr"><span></span></div> 
    <div class="row-fluid">
        ...
        <thead>
        <tr>
        <th width="60">Date</th><th>Query ID</th><th width="60">Status</th>
        </tr>
        </thead>
        <tbody>
        <?php 
        //===================================Second Month========================================
        $sql_three = 'SELECT * FROM '.TICKET_TABLE.' '.'WHERE isquery = "1" AND  month(created)='.$month.' AND  status = "open"  '.$dept_add.'';
        $sql_three .= $new_query_added;
        ...
        <tfoot>
        <tr>
        <td colspan="3" align="right"><a href="queries.php"><button class="btn btn-small">More...</button></a></td>
        </tr>
        </tfoot>
        </table>
        </div>  
        ...
        <?php 
        //===================================Second Month========================================
        $sql_three = 'SELECT * FROM '.TICKET_TABLE.' '.'WHERE isquery = "1" AND  month(created)='.$month.' AND  status = "open"  AND isaccepted = "0" '.$dept_add.'';
        ...
        <tfoot>
        <tr>
        <td colspan="3" align="right"><a href="queries.php?status=lodged"><button class="btn btn-small">More...</button></a></td>
        </tr>
        </tfoot>
        </table>
        </div>    
        ...
        <?php 
        //===================================Second Month========================================
        $sql_three = 'SELECT * FROM '.TICKET_TABLE.' '.'WHERE isquery = "1" AND  month(created)='.$month.' AND  status = "open"   AND isaccepted = "1" '.$dept_add.'';
