...
if($staffId && ($staffId==$thisstaff->getId())) { //My Complaints
  ...
  //$qwhere.=' AND ticket.isquery = 0 ';
    $qwhere .=" AND ticket.isquery = 0 AND ticket.isaccepted=1 AND TIMESTAMPDIFF(DAY,ticket.created,NOW())>=4 AND TIMESTAMPDIFF(DAY,ticket.created,NOW())<=10 AND ticket.isrejected<>'1' AND ticket.ticket_id>=4100 AND ticket.status='open' AND ticket.ticket_id NOT IN (select ticket_id from sdms_ticket_thread where thread_type='R') "; 
...
?>
            <tr id="<?php echo $row['ticket_id']; ?>" >
              ...
              <?php } ?>
              <td <?php echo $alret; ?> title="<?php echo $row['email']; ?>" nowrap><a class="Icon <?php echo strtolower($row['source']); ?>Ticket ticketPreview" title="Preview Complaint" href="tickets.php?id=<?php echo $row['ticket_id']; ?>"><?php echo $tid; ?></a></td>
              ...
              <td <?php echo $alret; ?>><?php if($thisstaff->canCreateTickets() && !$thisstaff->isAdmin()) {?>
                ...
                href="tickets.php?id=<?php echo $row['ticket_id']; ?>"><?php echo $subject; ?></a> &nbsp; <?php echo ($threadcount>1)?" <small>($threadcount)</small>&nbsp;":''?> <?php echo $row['attachments']?"<span class='Icon file'>&nbsp;</span>":''; ?></td>
              <?php }?>
	...
	{?>
              <td nowrap <?php echo $alret; ?>>&nbsp;
                ...
                <a href="<?php echo $row['ticket_voice']; ?>" target="_blank"><img src="img/play.png" /></a> <a href="force_download.php?file=<?php echo $row['ticket_voice'];?>"  ><img src="img/download.png" /></a> 
  ...
  <div class="row-fluid">
    <div class="span6" style="width:96.718%;">
      ...
      <p style="text-align:center;" id="actions">
        ...
        <?php //echo '<a class="btn " href="?a=export&h='.$hash.'&status='.$_REQUEST['status'] .'">';?><!--Export--><?php //echo '</a>'; ?> </p>
      <?php } }?>
      <?php }?>
    </div>
  </div>
