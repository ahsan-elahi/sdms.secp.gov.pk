...
$checkdate=date('Y-m-d', strtotime(date('Y-m-d').' -7 Weekday'));
$sql="SELECT * FROM `sdms_ticket` WHERE `isaccepted`=1 AND DATE(created)='".$checkdate."' AND isrejected<>'1' AND isquery='0' AND ticket_id>=4100 AND status='open' AND ticket_id NOT IN (select ticket_id from sdms_ticket_thread where thread_type='R')";
...
{
		//$row['staff_id']
$depresult = mysqli_query($conn, "select * from sdms_staff where isfocalperson='1' AND dept_id='".$row['dept_id']."' AND isactive='1'");
...
/****************************************************************15 Days *******************/
$checkdate=date('Y-m-d', strtotime(date('Y-m-d').' -15 Weekday'));
$sql="SELECT * FROM `sdms_ticket` WHERE `isaccepted`=1 AND DATE(created)='".$checkdate."' AND isrejected<>'1' AND isquery='0' AND ticket_id>=4100 AND status='open' AND ticket_id NOT IN (select ticket_id from sdms_ticket_thread where thread_type='R')";
$result = mysqli_query($conn, $sql);
...
/*****************************30 Days ********************************************/
$checkdate=date('Y-m-d', strtotime(date('Y-m-d').' -30 Weekday'));
$sql="SELECT * FROM `sdms_ticket` WHERE `isaccepted`=1 AND DATE(created)='".$checkdate."' AND isrejected<>'1' AND isquery='0' AND ticket_id>=4100 AND status='open' AND ticket_id NOT IN (select ticket_id from sdms_ticket_thread where thread_type='R')";
$result = mysqli_query($conn, $sql);
...
/*************************************************45 Days *************************************************************/
$checkdate=date('Y-m-d', strtotime(date('Y-m-d').' -45 Weekday'));
$sql="SELECT * FROM `sdms_ticket` WHERE `isaccepted`=1 AND DATE(created)='".$checkdate."' AND isrejected<>'1' AND ticket_id>=4100 AND status='open' AND isquery='0' AND ticket_id NOT IN (select ticket_id from sdms_ticket_thread where thread_type='R')";
$result = mysqli_query($conn, $sql);
...
{
		//$row['staff_id']
$depcomresult = mysqli_query($conn, "select * from sdmc_commissioner where dept_id='".$row['dept_id']."' AND isactive='1'");
